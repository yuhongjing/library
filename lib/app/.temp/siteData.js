export const siteData = {
  "title": "",
  "description": "",
  "base": "/library/",
  "pages": [
    {
      "key": "v-5334d1e448f95",
      "path": "/",
      "lastUpdated": 1572251081000,
      "title": "Home",
      "frontmatter": {
        "home": true,
        "heroImage": "/hero.png",
        "actionText": "GO →",
        "actionLink": "/notebook/school/read",
        "features": [
          {
            "title": "网站说明",
            "details": "本网站起源于小裕希望今后小裕能一帆风顺。"
          },
          {
            "title": "问题反馈",
            "details": "如果对文档内容有所疑惑，请在本项目的issue中反馈即可。"
          },
          {
            "title": "联系方式",
            "details": "QQ 1240945408"
          }
        ],
        "footer": "Copyright © 2019-present 余鸿靖"
      }
    },
    {
      "key": "v-3f21f6a85dd3e",
      "path": "/blog/article/BATFighting.html",
      "lastUpdated": 1574220385000,
      "title": "博客第一篇文章",
      "frontmatter": {
        "title": "博客第一篇文章"
      }
    },
    {
      "key": "v-1c1bde991b723",
      "path": "/blog/article/five-years-plan.html",
      "lastUpdated": 1571138319000,
      "title": "五年的计划",
      "headers": [
        {
          "level": 2,
          "title": "五年的计划",
          "slug": "五年的计划"
        },
        {
          "level": 2,
          "title": "前文",
          "slug": "前文"
        },
        {
          "level": 2,
          "title": "正文",
          "slug": "正文"
        },
        {
          "level": 2,
          "title": "结言",
          "slug": "结言"
        }
      ],
      "frontmatter": {
        "title": "五年的计划",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-c7020b9c68504",
      "path": "/blog/article/read.html",
      "lastUpdated": 1574220385000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-5483b7510880f",
      "path": "/blog/article/%E4%BA%BA%E6%98%AF%E6%80%8E%E4%B9%88%E5%BA%9F%E6%8E%89%E7%9A%84.html",
      "lastUpdated": 1574220385000,
      "title": "人是怎么废掉的",
      "headers": [
        {
          "level": 2,
          "title": "沉溺于「轻易获得高成就感」的事情",
          "slug": "沉溺于「轻易获得高成就感」的事情"
        },
        {
          "level": 2,
          "title": "只接受「低信息密度」的信息源",
          "slug": "只接受「低信息密度」的信息源"
        },
        {
          "level": 2,
          "title": "习惯用「错位成就感」麻痹自己",
          "slug": "习惯用「错位成就感」麻痹自己"
        },
        {
          "level": 2,
          "title": "过度依赖「既有可行路径」",
          "slug": "过度依赖「既有可行路径」"
        },
        {
          "level": 2,
          "title": "封闭「强化学习」的通道",
          "slug": "封闭「强化学习」的通道"
        },
        {
          "level": 2,
          "title": "如果这样就真的废了",
          "slug": "如果这样就真的废了"
        },
        {
          "level": 2,
          "title": "总结",
          "slug": "总结"
        }
      ],
      "frontmatter": {
        "title": "人是怎么废掉的"
      }
    },
    {
      "key": "v-a20a65e1a8917",
      "path": "/blog/article/%E7%A7%8B%E6%8B%9B%E6%80%BB%E7%BB%93.html",
      "lastUpdated": 1571138319000,
      "title": "秋招记录",
      "headers": [
        {
          "level": 2,
          "title": "明志",
          "slug": "明志"
        },
        {
          "level": 2,
          "title": "秋招面试情况",
          "slug": "秋招面试情况"
        },
        {
          "level": 2,
          "title": "面筋",
          "slug": "面筋"
        },
        {
          "level": 2,
          "title": "总结",
          "slug": "总结"
        },
        {
          "level": 2,
          "title": "纪念",
          "slug": "纪念"
        }
      ],
      "frontmatter": {
        "title": "秋招记录"
      }
    },
    {
      "key": "v-42b3fa0ba20a5",
      "path": "/blog/article/%E7%AC%AC%E4%BA%8C%E4%B8%AA%E7%9B%AE%E6%A0%87.html",
      "lastUpdated": 1574220385000,
      "title": "第二个目标",
      "frontmatter": {
        "title": "第二个目标"
      }
    },
    {
      "key": "v-b4c2d7e3cc25e",
      "path": "/blog/command/Alfred4.html",
      "lastUpdated": 1571138319000,
      "title": "Alfred 4使用手册",
      "headers": [
        {
          "level": 2,
          "title": "Alfred 4快速启动器",
          "slug": "alfred-4快速启动器"
        },
        {
          "level": 2,
          "title": "应用搜索",
          "slug": "应用搜索"
        },
        {
          "level": 2,
          "title": "文件或目录搜索",
          "slug": "文件或目录搜索"
        },
        {
          "level": 2,
          "title": "文本内容搜索",
          "slug": "文本内容搜索"
        },
        {
          "level": 2,
          "title": "标记搜索",
          "slug": "标记搜索"
        },
        {
          "level": 2,
          "title": "快捷网页搜索",
          "slug": "快捷网页搜索"
        },
        {
          "level": 2,
          "title": "书签搜索",
          "slug": "书签搜索"
        },
        {
          "level": 2,
          "title": "计算器",
          "slug": "计算器"
        },
        {
          "level": 2,
          "title": "词典搜索",
          "slug": "词典搜索"
        },
        {
          "level": 2,
          "title": "剪切板搜索",
          "slug": "剪切板搜索"
        },
        {
          "level": 2,
          "title": "系统常用命令快捷操作",
          "slug": "系统常用命令快捷操作"
        },
        {
          "level": 2,
          "title": "唤醒终端并执行命令",
          "slug": "唤醒终端并执行命令"
        }
      ],
      "frontmatter": {
        "title": "Alfred 4使用手册",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-ff4ddaf37d61e",
      "path": "/blog/command/Mac.html",
      "lastUpdated": 1574245566000,
      "title": "Mac使用方法整理",
      "headers": [
        {
          "level": 2,
          "title": "多屏时，如何固定dock的位置",
          "slug": "多屏时，如何固定dock的位置"
        },
        {
          "level": 2,
          "title": "快捷键",
          "slug": "快捷键"
        }
      ],
      "frontmatter": {
        "title": "Mac使用方法整理"
      }
    },
    {
      "key": "v-0a1e9e81bde88",
      "path": "/blog/command/Tmux.html",
      "lastUpdated": 1571138319000,
      "title": "Tmux使用手册",
      "headers": [
        {
          "level": 2,
          "title": "Tmux使用手册",
          "slug": "tmux使用手册"
        }
      ],
      "frontmatter": {
        "title": "Tmux使用手册",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-ed9e524282f9a",
      "path": "/blog/command/git.html",
      "lastUpdated": 1572934205000,
      "title": "git命令",
      "headers": [
        {
          "level": 2,
          "title": "配置Git相关命令",
          "slug": "配置git相关命令"
        },
        {
          "level": 2,
          "title": "增加删除文件相关命令",
          "slug": "增加删除文件相关命令"
        },
        {
          "level": 2,
          "title": "远程仓库相关命令",
          "slug": "远程仓库相关命令"
        },
        {
          "level": 2,
          "title": "文件提交相关命令",
          "slug": "文件提交相关命令"
        },
        {
          "level": 2,
          "title": "分支(branch)操作相关命令",
          "slug": "分支-branch-操作相关命令"
        },
        {
          "level": 2,
          "title": "版本(tag)操作相关命令",
          "slug": "版本-tag-操作相关命令"
        },
        {
          "level": 2,
          "title": "查看信息相关命令",
          "slug": "查看信息相关命令"
        },
        {
          "level": 2,
          "title": "撤销相关命令",
          "slug": "撤销相关命令"
        },
        {
          "level": 2,
          "title": "忽略提交文件、文件夹到远程仓库",
          "slug": "忽略提交文件、文件夹到远程仓库"
        },
        {
          "level": 2,
          "title": "其他",
          "slug": "其他"
        }
      ],
      "frontmatter": {
        "title": "git命令",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-79dd7916bd32f",
      "path": "/blog/command/iterm2.html",
      "lastUpdated": 1572934205000,
      "title": "iTerm2快捷键",
      "headers": [
        {
          "level": 2,
          "title": "标签相关快捷键",
          "slug": "标签相关快捷键"
        },
        {
          "level": 2,
          "title": "分屏相关快捷键",
          "slug": "分屏相关快捷键"
        },
        {
          "level": 2,
          "title": "文本操作快捷键",
          "slug": "文本操作快捷键"
        },
        {
          "level": 2,
          "title": "其他命令",
          "slug": "其他命令"
        }
      ],
      "frontmatter": {
        "title": "iTerm2快捷键",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-25a708e2dfffd",
      "path": "/blog/command/linux.html",
      "lastUpdated": 1571138319000,
      "title": "linux常用命令",
      "headers": [
        {
          "level": 2,
          "title": "常用的linux命令整理",
          "slug": "常用的linux命令整理"
        },
        {
          "level": 2,
          "title": "cd命令",
          "slug": "cd命令"
        },
        {
          "level": 2,
          "title": "ls命令",
          "slug": "ls命令"
        },
        {
          "level": 2,
          "title": "grep命令",
          "slug": "grep命令"
        },
        {
          "level": 2,
          "title": "find命令",
          "slug": "find命令"
        },
        {
          "level": 2,
          "title": "cp命令",
          "slug": "cp命令"
        },
        {
          "level": 2,
          "title": "mv命令",
          "slug": "mv命令"
        },
        {
          "level": 2,
          "title": "rm命令",
          "slug": "rm命令"
        },
        {
          "level": 2,
          "title": "ps命令",
          "slug": "ps命令"
        },
        {
          "level": 2,
          "title": "kill命令",
          "slug": "kill命令"
        },
        {
          "level": 2,
          "title": "killall命令",
          "slug": "killall命令"
        },
        {
          "level": 2,
          "title": "file命令",
          "slug": "file命令"
        },
        {
          "level": 2,
          "title": "tar命令",
          "slug": "tar命令"
        },
        {
          "level": 2,
          "title": "cat命令",
          "slug": "cat命令"
        },
        {
          "level": 2,
          "title": "chgrp命令",
          "slug": "chgrp命令"
        },
        {
          "level": 2,
          "title": "chown命令",
          "slug": "chown命令"
        },
        {
          "level": 2,
          "title": "chmod命令",
          "slug": "chmod命令"
        },
        {
          "level": 2,
          "title": "vim命令",
          "slug": "vim命令"
        },
        {
          "level": 2,
          "title": "gcc命令",
          "slug": "gcc命令"
        },
        {
          "level": 2,
          "title": "time命令",
          "slug": "time命令"
        }
      ],
      "frontmatter": {
        "title": "linux常用命令"
      }
    },
    {
      "key": "v-3d2eaadd057e8",
      "path": "/blog/command/markdown.html",
      "lastUpdated": 1571138319000,
      "title": "markdown语法",
      "headers": [
        {
          "level": 2,
          "title": "markdown语法大全",
          "slug": "markdown语法大全"
        },
        {
          "level": 2,
          "title": "标题",
          "slug": "标题"
        },
        {
          "level": 2,
          "title": "字体",
          "slug": "字体"
        },
        {
          "level": 2,
          "title": "引用",
          "slug": "引用"
        },
        {
          "level": 2,
          "title": "分割线",
          "slug": "分割线"
        },
        {
          "level": 2,
          "title": "图片",
          "slug": "图片"
        },
        {
          "level": 2,
          "title": "超链接",
          "slug": "超链接"
        },
        {
          "level": 2,
          "title": "列表",
          "slug": "列表"
        },
        {
          "level": 2,
          "title": "表格",
          "slug": "表格"
        },
        {
          "level": 2,
          "title": "代码",
          "slug": "代码"
        },
        {
          "level": 2,
          "title": "流程图",
          "slug": "流程图"
        }
      ],
      "frontmatter": {
        "title": "markdown语法",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-9e87a90b2e448",
      "path": "/blog/command/read.html",
      "lastUpdated": 1574245566000,
      "title": "介绍",
      "headers": [
        {
          "level": 2,
          "title": "命令语言类",
          "slug": "命令语言类"
        },
        {
          "level": 2,
          "title": "软件快捷键类",
          "slug": "软件快捷键类"
        }
      ],
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-cb7234af31b18",
      "path": "/blog/command/vim.html",
      "lastUpdated": 1571138319000,
      "title": "Vim命令整理",
      "headers": [
        {
          "level": 2,
          "title": "整理Vim相关的命令和操作",
          "slug": "整理vim相关的命令和操作"
        },
        {
          "level": 2,
          "title": "Vim的几种模式",
          "slug": "vim的几种模式"
        },
        {
          "level": 2,
          "title": "启动Vim(使用Vim打开文件)",
          "slug": "启动vim-使用vim打开文件"
        },
        {
          "level": 2,
          "title": "文档操作(例如退出Vim)",
          "slug": "文档操作-例如退出vim"
        },
        {
          "level": 2,
          "title": "光标移动",
          "slug": "光标移动"
        },
        {
          "level": 2,
          "title": "查找与替换",
          "slug": "查找与替换"
        },
        {
          "level": 2,
          "title": "正则表达式",
          "slug": "正则表达式"
        }
      ],
      "frontmatter": {
        "title": "Vim命令整理"
      }
    },
    {
      "key": "v-e54e80584265f",
      "path": "/blog/command/vscode.html",
      "lastUpdated": 1571138319000,
      "title": "VS Code技巧",
      "headers": [
        {
          "level": 2,
          "title": "基本技巧",
          "slug": "基本技巧"
        },
        {
          "level": 3,
          "title": "Command Palette | 命令面板",
          "slug": "command-palette-命令面板"
        },
        {
          "level": 3,
          "title": "Quick open | 快速打开",
          "slug": "quick-open-快速打开"
        },
        {
          "level": 3,
          "title": "Displays errors and warnings | 显示错误和警告",
          "slug": "displays-errors-and-warnings-显示错误和警告"
        },
        {
          "level": 3,
          "title": "Open Settings | 打开配置文件",
          "slug": "open-settings-打开配置文件"
        },
        {
          "level": 3,
          "title": "Open terminal | 打开终端",
          "slug": "open-terminal-打开终端"
        },
        {
          "level": 3,
          "title": "File and folder management | 文件栏显示",
          "slug": "file-and-folder-management-文件栏显示"
        },
        {
          "level": 3,
          "title": "Global Search | 全局搜索",
          "slug": "global-search-全局搜索"
        },
        {
          "level": 2,
          "title": "高级技巧",
          "slug": "高级技巧"
        },
        {
          "level": 3,
          "title": "代码块选择",
          "slug": "代码块选择"
        },
        {
          "level": 3,
          "title": "文件内快速定位代码",
          "slug": "文件内快速定位代码"
        },
        {
          "level": 3,
          "title": "项目内快速定位代码",
          "slug": "项目内快速定位代码"
        },
        {
          "level": 3,
          "title": "上下行复制",
          "slug": "上下行复制"
        },
        {
          "level": 3,
          "title": "本行内容上下移动",
          "slug": "本行内容上下移动"
        },
        {
          "level": 3,
          "title": "选择当前行",
          "slug": "选择当前行"
        },
        {
          "level": 3,
          "title": "跳转到文件顶部/尾部",
          "slug": "跳转到文件顶部-尾部"
        },
        {
          "level": 3,
          "title": "代码折叠",
          "slug": "代码折叠"
        },
        {
          "level": 3,
          "title": "打开多个窗口",
          "slug": "打开多个窗口"
        }
      ],
      "frontmatter": {
        "title": "VS Code技巧",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-ce83386cd29df",
      "path": "/blog/other/read.html",
      "lastUpdated": 1571294131000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-3d5bff0966e14",
      "path": "/blog/other/%E4%BB%A3%E7%A0%81%E4%BA%A7%E5%87%BA.html",
      "lastUpdated": 1571138319000,
      "title": "代码产出",
      "headers": [
        {
          "level": 2,
          "title": "登录服务器",
          "slug": "登录服务器"
        },
        {
          "level": 2,
          "title": "下载产出",
          "slug": "下载产出"
        }
      ],
      "frontmatter": {
        "title": "代码产出",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-a0ffe885e6e5a",
      "path": "/blog/software/Chrome%E6%8F%92%E4%BB%B6.html",
      "lastUpdated": 1573615737000,
      "title": "Chrome插件",
      "headers": [
        {
          "level": 2,
          "title": "Chrome插件收集",
          "slug": "chrome插件收集"
        },
        {
          "level": 2,
          "title": "编程调试",
          "slug": "编程调试"
        },
        {
          "level": 2,
          "title": "实用工具",
          "slug": "实用工具"
        }
      ],
      "frontmatter": {
        "title": "Chrome插件"
      }
    },
    {
      "key": "v-b72f4e55b547b",
      "path": "/blog/software/Mac%E5%BF%85%E5%A4%87%E5%BA%94%E7%94%A8.html",
      "lastUpdated": 1575526467000,
      "title": "Mac必备应用",
      "headers": [
        {
          "level": 2,
          "title": "Mac必备应用汇总",
          "slug": "mac必备应用汇总"
        },
        {
          "level": 2,
          "title": "编辑器",
          "slug": "编辑器"
        },
        {
          "level": 2,
          "title": "前端工具",
          "slug": "前端工具"
        },
        {
          "level": 2,
          "title": "系统工具",
          "slug": "系统工具"
        },
        {
          "level": 2,
          "title": "其他工具",
          "slug": "其他工具"
        }
      ],
      "frontmatter": {
        "title": "Mac必备应用",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-e003759f82f79",
      "path": "/blog/software/Node%E6%8F%92%E4%BB%B6.html",
      "lastUpdated": 1573615737000,
      "title": "Node插件",
      "headers": [
        {
          "level": 2,
          "title": "NPM包收集",
          "slug": "npm包收集"
        },
        {
          "level": 2,
          "title": "调试",
          "slug": "调试"
        },
        {
          "level": 2,
          "title": "请求",
          "slug": "请求"
        },
        {
          "level": 2,
          "title": "自己的NPM包",
          "slug": "自己的npm包"
        }
      ],
      "frontmatter": {
        "title": "Node插件"
      }
    },
    {
      "key": "v-e4a7d283dbf08",
      "path": "/blog/software/Vscode%E5%BF%85%E5%A4%87%E6%8F%92%E4%BB%B6.html",
      "lastUpdated": 1575173055000,
      "title": "VS Code必备插件",
      "headers": [
        {
          "level": 2,
          "title": "VS Code必备插件",
          "slug": "vs-code必备插件"
        },
        {
          "level": 2,
          "title": "代码规范类插件",
          "slug": "代码规范类插件"
        },
        {
          "level": 2,
          "title": "文件高亮类插件",
          "slug": "文件高亮类插件"
        },
        {
          "level": 2,
          "title": "贴心功能类插件",
          "slug": "贴心功能类插件"
        },
        {
          "level": 2,
          "title": "主题插件",
          "slug": "主题插件"
        },
        {
          "level": 2,
          "title": "其他插件",
          "slug": "其他插件"
        }
      ],
      "frontmatter": {
        "title": "VS Code必备插件",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-e70c384664982",
      "path": "/blog/software/Webpack%E6%8F%92%E4%BB%B6.html",
      "lastUpdated": 1572934205000,
      "title": "Webpack插件",
      "headers": [
        {
          "level": 2,
          "title": "Webpack插件收集",
          "slug": "webpack插件收集"
        },
        {
          "level": 2,
          "title": "调试插件",
          "slug": "调试插件"
        }
      ],
      "frontmatter": {
        "title": "Webpack插件"
      }
    },
    {
      "key": "v-38d2817046f65",
      "path": "/blog/software/read.html",
      "lastUpdated": 1572934205000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-7ee166a261f4",
      "path": "/blog/tools/PicGo%E9%85%8D%E5%90%88Github%E5%9B%BE%E5%BA%8A.html",
      "lastUpdated": 1575526467000,
      "title": "PicGo配合GitHub图床",
      "headers": [
        {
          "level": 2,
          "title": "PicGo下载",
          "slug": "picgo下载"
        },
        {
          "level": 2,
          "title": "GitHub配置",
          "slug": "github配置"
        },
        {
          "level": 3,
          "title": "建立一个公共仓库或使用已有的仓库",
          "slug": "建立一个公共仓库或使用已有的仓库"
        },
        {
          "level": 3,
          "title": "配置Personal access tokens",
          "slug": "配置personal-access-tokens"
        },
        {
          "level": 2,
          "title": "配置PicGo",
          "slug": "配置picgo"
        },
        {
          "level": 3,
          "title": "选择GitHub图床",
          "slug": "选择github图床"
        },
        {
          "level": 3,
          "title": "其余",
          "slug": "其余"
        },
        {
          "level": 2,
          "title": "总结",
          "slug": "总结"
        }
      ],
      "frontmatter": {
        "title": "PicGo配合GitHub图床"
      }
    },
    {
      "key": "v-b57dab27e2cd8",
      "path": "/blog/tools/git%E4%BF%AE%E6%94%B9%E4%BD%9C%E8%80%85.html",
      "lastUpdated": 1571152849000,
      "title": "Git修改作者",
      "headers": [
        {
          "level": 3,
          "title": "Git修改作者",
          "slug": "git修改作者"
        },
        {
          "level": 3,
          "title": "修改命令",
          "slug": "修改命令"
        },
        {
          "level": 3,
          "title": "强推远程仓库",
          "slug": "强推远程仓库"
        },
        {
          "level": 3,
          "title": "错误解决",
          "slug": "错误解决"
        }
      ],
      "frontmatter": {
        "title": "Git修改作者",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-ff3c639241c76",
      "path": "/blog/tools/git%E8%BF%9E%E6%8E%A5%E5%A4%9A%E4%BB%93%E5%BA%93.html",
      "lastUpdated": 1571152849000,
      "title": "Git连接多托管网站",
      "headers": [
        {
          "level": 3,
          "title": "Git同一台电脑连接多个远程仓库",
          "slug": "git同一台电脑连接多个远程仓库"
        },
        {
          "level": 3,
          "title": "创建新的密匙，并指定密匙的名称，例如idrsaX",
          "slug": "创建新的密匙，并指定密匙的名称，例如id-rsa-x"
        },
        {
          "level": 3,
          "title": "在~/.ssh/文件夹下创建一个config文件",
          "slug": "在-ssh-文件夹下创建一个config文件"
        },
        {
          "level": 3,
          "title": "编写config文件，配置不同的仓库指向不同的密钥文件",
          "slug": "编写config文件，配置不同的仓库指向不同的密钥文件"
        },
        {
          "level": 3,
          "title": "查看SSH密匙的值，分别添加到对应的Git账户中",
          "slug": "查看ssh密匙的值，分别添加到对应的git账户中"
        },
        {
          "level": 3,
          "title": "清空本地SSH缓存，添加新的SSH密匙到SSH agent中",
          "slug": "清空本地ssh缓存，添加新的ssh密匙到ssh-agent中"
        },
        {
          "level": 3,
          "title": "确认新密匙是否添加成功",
          "slug": "确认新密匙是否添加成功"
        },
        {
          "level": 3,
          "title": "测试ssh链接",
          "slug": "测试ssh链接"
        },
        {
          "level": 3,
          "title": "取消git全局用户名/邮箱的设置，设置独立的用户名/邮箱",
          "slug": "取消git全局用户名-邮箱的设置，设置独立的用户名-邮箱"
        },
        {
          "level": 3,
          "title": "查看git项目的配置",
          "slug": "查看git项目的配置"
        },
        {
          "level": 3,
          "title": "命令行进入项目目录，重建origin",
          "slug": "命令行进入项目目录，重建origin"
        }
      ],
      "frontmatter": {
        "title": "Git连接多托管网站",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-5cffbbcb1659f",
      "path": "/blog/tools/read.html",
      "lastUpdated": 1575534003000,
      "title": "介绍",
      "headers": [
        {
          "level": 2,
          "title": "web开发",
          "slug": "web开发"
        },
        {
          "level": 2,
          "title": "Git相关",
          "slug": "git相关"
        },
        {
          "level": 2,
          "title": "系统相关",
          "slug": "系统相关"
        },
        {
          "level": 2,
          "title": "其余",
          "slug": "其余"
        }
      ],
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-674de1ed52c9f",
      "path": "/blog/tools/win10%E6%BF%80%E6%B4%BB.html",
      "lastUpdated": 1571138319000,
      "title": "Win10激活",
      "headers": [
        {
          "level": 3,
          "title": "Win10家庭版和专业版的激活方法",
          "slug": "win10家庭版和专业版的激活方法"
        },
        {
          "level": 3,
          "title": "ID码",
          "slug": "id码"
        },
        {
          "level": 3,
          "title": "安装",
          "slug": "安装"
        }
      ],
      "frontmatter": {
        "title": "Win10激活",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-a4e7843697a43",
      "path": "/blog/tools/%E7%A7%BB%E5%8A%A8%E7%AB%AF%E8%B0%83%E8%AF%95.html",
      "lastUpdated": 1571138319000,
      "title": "移动端调试方法",
      "headers": [
        {
          "level": 2,
          "title": "移动端控制台",
          "slug": "移动端控制台"
        },
        {
          "level": 2,
          "title": "手机端访问电脑端本地项目",
          "slug": "手机端访问电脑端本地项目"
        }
      ],
      "frontmatter": {
        "title": "移动端调试方法",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-945af6d838dd9",
      "path": "/code/loadsh/array.html",
      "lastUpdated": 1571138319000,
      "title": "Array",
      "headers": [
        {
          "level": 2,
          "title": "chunk",
          "slug": "chunk"
        },
        {
          "level": 2,
          "title": "compact",
          "slug": "compact"
        },
        {
          "level": 2,
          "title": "concat",
          "slug": "concat"
        },
        {
          "level": 2,
          "title": "difference",
          "slug": "difference"
        }
      ]
    },
    {
      "key": "v-c594b39361f88",
      "path": "/code/loadsh/collection.html",
      "lastUpdated": 1571138319000,
      "title": "Collection"
    },
    {
      "key": "v-cddb91b074497",
      "path": "/code/loadsh/date.html",
      "lastUpdated": 1571138319000,
      "title": "Date"
    },
    {
      "key": "v-024565ce72cae",
      "path": "/code/loadsh/function.html",
      "lastUpdated": 1571138319000,
      "title": "Function"
    },
    {
      "key": "v-cc55ccceb0999",
      "path": "/code/loadsh/lang.html",
      "lastUpdated": 1571138319000,
      "title": "Lang"
    },
    {
      "key": "v-aa2dfa3cc9a84",
      "path": "/code/loadsh/math.html",
      "lastUpdated": 1571138319000,
      "title": "Math"
    },
    {
      "key": "v-023dfc378ffa3",
      "path": "/code/loadsh/methods.html",
      "lastUpdated": 1571138319000,
      "title": "Methods"
    },
    {
      "key": "v-962c4b8df84d4",
      "path": "/code/loadsh/number.html",
      "lastUpdated": 1571138319000,
      "title": "Number"
    },
    {
      "key": "v-a6ad8ba0b07ae",
      "path": "/code/loadsh/object.html",
      "lastUpdated": 1571138319000,
      "title": "Object"
    },
    {
      "key": "v-406cbc1ab44c3",
      "path": "/code/loadsh/properties.html",
      "lastUpdated": 1571138319000,
      "title": "Properties"
    },
    {
      "key": "v-c5505aaca5b77",
      "path": "/code/loadsh/read.html",
      "lastUpdated": 1571138319000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-2a9aadc44c401",
      "path": "/code/loadsh/seq.html",
      "lastUpdated": 1571138319000,
      "title": "Seq"
    },
    {
      "key": "v-16d9413035fa6",
      "path": "/code/loadsh/string.html",
      "lastUpdated": 1571138319000,
      "title": "String"
    },
    {
      "key": "v-f755e81039c95",
      "path": "/code/loadsh/util.html",
      "lastUpdated": 1571138319000,
      "title": "Util"
    },
    {
      "key": "v-dbcb3bf8cfde5",
      "path": "/code/webpack/read.html",
      "lastUpdated": 1571138319000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-a5b6db934f7e3",
      "path": "/code/zepto/read.html",
      "lastUpdated": 1571138319000,
      "title": "介绍",
      "headers": [
        {
          "level": 2,
          "title": "源码版本",
          "slug": "源码版本"
        },
        {
          "level": 2,
          "title": "介绍",
          "slug": "介绍"
        },
        {
          "level": 2,
          "title": "官方手册",
          "slug": "官方手册"
        }
      ],
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-2745726a57d1",
      "path": "/code/zepto/structure.html",
      "lastUpdated": 1571138319000,
      "title": "Zepto之架构",
      "headers": [
        {
          "level": 2,
          "title": "目录文件及作用",
          "slug": "目录文件及作用"
        }
      ],
      "frontmatter": {
        "title": "Zepto之架构",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-db9dd06bf6616",
      "path": "/knowledge/algorithm/read.html",
      "lastUpdated": 1571138319000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-5095abb8dc6f3",
      "path": "/knowledge/algorithm/%E5%86%92%E6%B3%A1%E6%8E%92%E5%BA%8F.html",
      "lastUpdated": 1572934205000,
      "title": "冒泡排序",
      "headers": [
        {
          "level": 2,
          "title": "冒泡排序",
          "slug": "冒泡排序"
        }
      ],
      "frontmatter": {
        "title": "冒泡排序",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-cf62da8bf4c4",
      "path": "/knowledge/algorithm/%E5%A0%86%E6%8E%92%E5%BA%8F.html",
      "lastUpdated": 1572934205000,
      "title": "堆排序",
      "headers": [
        {
          "level": 2,
          "title": "堆排序",
          "slug": "堆排序"
        }
      ],
      "frontmatter": {
        "title": "堆排序",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-c04bc4865a19",
      "path": "/knowledge/algorithm/%E5%A4%8D%E6%9D%82%E5%BA%A6.html",
      "lastUpdated": 1572934205000,
      "title": "复杂度",
      "headers": [
        {
          "level": 2,
          "title": "复杂度",
          "slug": "复杂度"
        }
      ],
      "frontmatter": {
        "title": "复杂度",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-ce936e4aec446",
      "path": "/knowledge/algorithm/%E5%BD%92%E5%B9%B6%E6%8E%92%E5%BA%8F.html",
      "lastUpdated": 1572934205000,
      "title": "归并排序",
      "headers": [
        {
          "level": 2,
          "title": "归并排序",
          "slug": "归并排序"
        }
      ],
      "frontmatter": {
        "title": "归并排序",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-815713e2dfdd8",
      "path": "/knowledge/algorithm/%E5%BF%AB%E9%80%9F%E6%8E%92%E5%BA%8F.html",
      "lastUpdated": 1572934205000,
      "title": "快速排序",
      "headers": [
        {
          "level": 2,
          "title": "快速排序",
          "slug": "快速排序"
        }
      ],
      "frontmatter": {
        "title": "快速排序",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-2aa5b505380e8",
      "path": "/knowledge/algorithm/%E6%8E%92%E5%BA%8F%E6%A6%82%E8%A7%88.html",
      "lastUpdated": 1572934205000,
      "title": "排序-概览",
      "headers": [
        {
          "level": 2,
          "title": "排序-概览",
          "slug": "排序-概览"
        }
      ],
      "frontmatter": {
        "title": "排序-概览",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-8423a1b3eb5ab",
      "path": "/knowledge/algorithm/%E6%8F%92%E5%85%A5%E6%8E%92%E5%BA%8F.html",
      "lastUpdated": 1572934205000,
      "title": "插入排序",
      "headers": [
        {
          "level": 2,
          "title": "插入排序",
          "slug": "插入排序"
        }
      ],
      "frontmatter": {
        "title": "插入排序",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-1ed97a3ebb71a",
      "path": "/knowledge/algorithm/%E6%95%B0%E7%BB%84%E9%80%86%E5%BA%8F%E5%AF%B9.html",
      "lastUpdated": 1572934205000,
      "title": "数组逆序对",
      "headers": [
        {
          "level": 2,
          "title": "数组逆序对",
          "slug": "数组逆序对"
        }
      ],
      "frontmatter": {
        "title": "数组逆序对",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-a5aff704102ba",
      "path": "/knowledge/algorithm/%E9%80%89%E6%8B%A9%E6%8E%92%E5%BA%8F.html",
      "lastUpdated": 1572934205000,
      "title": "选择排序",
      "headers": [
        {
          "level": 2,
          "title": "选择排序",
          "slug": "选择排序"
        }
      ],
      "frontmatter": {
        "title": "选择排序",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-8c157c181bbc",
      "path": "/knowledge/architecture/read.html",
      "lastUpdated": 1572934205000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-14bce7e1c8f4c",
      "path": "/knowledge/backend/php.html",
      "lastUpdated": 1571138319000,
      "title": "php",
      "frontmatter": {
        "title": "php"
      }
    },
    {
      "key": "v-d11b8f9191712",
      "path": "/knowledge/backend/read.html",
      "lastUpdated": 1571138319000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-c127c3f58b2fd",
      "path": "/knowledge/config/c.html",
      "lastUpdated": 1574335933000,
      "title": "C语言",
      "headers": [
        {
          "level": 2,
          "title": "Mac系统下配置C环境",
          "slug": "mac系统下配置c环境"
        }
      ],
      "frontmatter": {
        "title": "C语言"
      }
    },
    {
      "key": "v-770f6ef5fd977",
      "path": "/knowledge/config/eslint.html",
      "lastUpdated": 1573635196000,
      "title": "ESLint",
      "headers": [
        {
          "level": 2,
          "title": "ESLint是个啥？",
          "slug": "eslint是个啥？"
        },
        {
          "level": 2,
          "title": "那该如何安装这个玩意呢？",
          "slug": "那该如何安装这个玩意呢？"
        },
        {
          "level": 2,
          "title": "大功告成",
          "slug": "大功告成"
        },
        {
          "level": 2,
          "title": "最后",
          "slug": "最后"
        }
      ],
      "frontmatter": {
        "title": "ESLint",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-ee6b4b78934ba",
      "path": "/knowledge/config/mysql.html",
      "lastUpdated": 1573629897000,
      "title": "MySQL",
      "headers": [
        {
          "level": 2,
          "title": "Mac系统下配置PHP环境",
          "slug": "mac系统下配置php环境"
        },
        {
          "level": 3,
          "title": "使用homebrew一句代码安装MySQL",
          "slug": "使用homebrew一句代码安装mysql"
        },
        {
          "level": 3,
          "title": "修改配置文件",
          "slug": "修改配置文件"
        },
        {
          "level": 3,
          "title": "启动服务",
          "slug": "启动服务"
        },
        {
          "level": 3,
          "title": "连接数据库",
          "slug": "连接数据库"
        }
      ],
      "frontmatter": {
        "title": "MySQL"
      }
    },
    {
      "key": "v-dcb5b136134a7",
      "path": "/knowledge/config/php.html",
      "lastUpdated": 1573629897000,
      "title": "PHP",
      "headers": [
        {
          "level": 2,
          "title": "Mac系统下配置PHP环境",
          "slug": "mac系统下配置php环境"
        },
        {
          "level": 3,
          "title": "Apache",
          "slug": "apache"
        },
        {
          "level": 3,
          "title": "PHP",
          "slug": "php"
        }
      ],
      "frontmatter": {
        "title": "PHP"
      }
    },
    {
      "key": "v-fac1a86761fd1",
      "path": "/knowledge/config/read.html",
      "lastUpdated": 1571138319000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-35ea9dcfeff13",
      "path": "/knowledge/database/MySQL%E4%BB%8B%E7%BB%8D.html",
      "lastUpdated": 1575173055000,
      "title": "MySQL简介"
    },
    {
      "key": "v-4b51b9da0fd2e",
      "path": "/knowledge/database/MySQL%E5%BC%80%E5%8F%91%E8%A7%84%E8%8C%83.html",
      "lastUpdated": 1575173055000,
      "title": "MySQL开发规范",
      "headers": [
        {
          "level": 2,
          "title": "数据库对象命名规范",
          "slug": "数据库对象命名规范"
        },
        {
          "level": 3,
          "title": "数据库对象",
          "slug": "数据库对象"
        },
        {
          "level": 3,
          "title": "数据库对象命名原则",
          "slug": "数据库对象命名原则"
        },
        {
          "level": 3,
          "title": "数据库命名规范",
          "slug": "数据库命名规范"
        },
        {
          "level": 3,
          "title": "表命名规范",
          "slug": "表命名规范"
        },
        {
          "level": 3,
          "title": "字段命名规范",
          "slug": "字段命名规范"
        },
        {
          "level": 3,
          "title": "用户命名规范",
          "slug": "用户命名规范"
        },
        {
          "level": 2,
          "title": "数据库对象设计规范",
          "slug": "数据库对象设计规范"
        },
        {
          "level": 3,
          "title": "存储引擎的选择",
          "slug": "存储引擎的选择"
        },
        {
          "level": 3,
          "title": "字符集的选择",
          "slug": "字符集的选择"
        },
        {
          "level": 3,
          "title": "表设计规范",
          "slug": "表设计规范"
        },
        {
          "level": 3,
          "title": "字段设计规范",
          "slug": "字段设计规范"
        },
        {
          "level": 3,
          "title": "索引设计规范",
          "slug": "索引设计规范"
        },
        {
          "level": 3,
          "title": "约束设计规范",
          "slug": "约束设计规范"
        },
        {
          "level": 3,
          "title": "SQL编写规范",
          "slug": "sql编写规范"
        }
      ]
    },
    {
      "key": "v-8332e8d9f5fbf",
      "path": "/knowledge/database/read.html",
      "lastUpdated": 1571138319000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-9dce052e2f24d",
      "path": "/knowledge/design/read.html",
      "lastUpdated": 1575534003000,
      "title": "介绍",
      "headers": [
        {
          "level": 2,
          "title": "基础原则",
          "slug": "基础原则"
        },
        {
          "level": 2,
          "title": "创建型模式",
          "slug": "创建型模式"
        },
        {
          "level": 2,
          "title": "行为型模式",
          "slug": "行为型模式"
        },
        {
          "level": 2,
          "title": "结构型模式",
          "slug": "结构型模式"
        }
      ],
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-b987614c7196f",
      "path": "/knowledge/design/solid.html",
      "lastUpdated": 1572934205000,
      "title": "S.O.L.I.D",
      "headers": [
        {
          "level": 2,
          "title": "单一职责原则",
          "slug": "单一职责原则"
        },
        {
          "level": 2,
          "title": "开闭原则",
          "slug": "开闭原则"
        },
        {
          "level": 2,
          "title": "里式替换原则",
          "slug": "里式替换原则"
        },
        {
          "level": 2,
          "title": "接口隔离原则",
          "slug": "接口隔离原则"
        },
        {
          "level": 2,
          "title": "依赖反转原则",
          "slug": "依赖反转原则"
        },
        {
          "level": 2,
          "title": "总结",
          "slug": "总结"
        }
      ],
      "frontmatter": {
        "title": "S.O.L.I.D",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-9832eaf828a08",
      "path": "/knowledge/design/%E4%BB%A3%E7%90%86%E6%A8%A1%E5%BC%8F.html",
      "lastUpdated": 1574249314000,
      "title": "代理模式",
      "headers": [
        {
          "level": 2,
          "title": "使用场景",
          "slug": "使用场景"
        },
        {
          "level": 2,
          "title": "种类",
          "slug": "种类"
        },
        {
          "level": 3,
          "title": "静态代理",
          "slug": "静态代理"
        },
        {
          "level": 3,
          "title": "动态代理",
          "slug": "动态代理"
        },
        {
          "level": 2,
          "title": "静态代理模式与装饰者模式的区别",
          "slug": "静态代理模式与装饰者模式的区别"
        },
        {
          "level": 2,
          "title": "总结",
          "slug": "总结"
        }
      ],
      "frontmatter": {
        "title": "代理模式"
      }
    },
    {
      "key": "v-03835b148d7b7",
      "path": "/knowledge/design/%E5%8E%9F%E5%9E%8B%E6%A8%A1%E5%BC%8F.html",
      "lastUpdated": 1575020017000,
      "title": "原型模式",
      "headers": [
        {
          "level": 2,
          "title": "适用场景",
          "slug": "适用场景"
        },
        {
          "level": 2,
          "title": "核心思想",
          "slug": "核心思想"
        },
        {
          "level": 2,
          "title": "举例说明",
          "slug": "举例说明"
        },
        {
          "level": 3,
          "title": "浅copy模式",
          "slug": "浅copy模式"
        },
        {
          "level": 3,
          "title": "深拷贝模式",
          "slug": "深拷贝模式"
        },
        {
          "level": 2,
          "title": "总结",
          "slug": "总结"
        }
      ],
      "frontmatter": {
        "title": "原型模式"
      }
    },
    {
      "key": "v-228b28656a517",
      "path": "/knowledge/design/%E5%A4%96%E8%A7%82%E6%A8%A1%E5%BC%8F.html",
      "lastUpdated": 1575534003000,
      "title": "外观模式",
      "headers": [
        {
          "level": 2,
          "title": "适用场景",
          "slug": "适用场景"
        },
        {
          "level": 2,
          "title": "核心思想",
          "slug": "核心思想"
        },
        {
          "level": 2,
          "title": "举例说明",
          "slug": "举例说明"
        },
        {
          "level": 2,
          "title": "总结",
          "slug": "总结"
        }
      ],
      "frontmatter": {
        "title": "外观模式"
      }
    },
    {
      "key": "v-65759c6ff1fe1",
      "path": "/knowledge/design/%E5%B7%A5%E5%8E%82%E6%96%B9%E6%B3%95%E6%A8%A1%E5%BC%8F.html",
      "lastUpdated": 1575020017000,
      "title": "工厂方法模式",
      "headers": [
        {
          "level": 2,
          "title": "适用场景",
          "slug": "适用场景"
        },
        {
          "level": 2,
          "title": "核心思想",
          "slug": "核心思想"
        },
        {
          "level": 2,
          "title": "举例说明",
          "slug": "举例说明"
        },
        {
          "level": 2,
          "title": "总结",
          "slug": "总结"
        }
      ],
      "frontmatter": {
        "title": "工厂方法模式"
      }
    },
    {
      "key": "v-79f55d90822a2",
      "path": "/knowledge/design/%E6%A8%A1%E6%9D%BF%E6%96%B9%E6%B3%95%E6%A8%A1%E5%BC%8F.html",
      "lastUpdated": 1575020017000,
      "title": "模板方法模式",
      "headers": [
        {
          "level": 2,
          "title": "适用场景",
          "slug": "适用场景"
        },
        {
          "level": 2,
          "title": "核心思想",
          "slug": "核心思想"
        },
        {
          "level": 2,
          "title": "举例说明",
          "slug": "举例说明"
        },
        {
          "level": 2,
          "title": "总结",
          "slug": "总结"
        }
      ],
      "frontmatter": {
        "title": "模板方法模式"
      }
    },
    {
      "key": "v-2d7e79ed7328b",
      "path": "/knowledge/design/%E7%AD%96%E7%95%A5%E6%A8%A1%E5%BC%8F.html",
      "lastUpdated": 1574161526000,
      "title": "策略模式",
      "headers": [
        {
          "level": 2,
          "title": "适用场景",
          "slug": "适用场景"
        },
        {
          "level": 2,
          "title": "核心思想",
          "slug": "核心思想"
        },
        {
          "level": 2,
          "title": "举例说明",
          "slug": "举例说明"
        },
        {
          "level": 2,
          "title": "总结",
          "slug": "总结"
        }
      ],
      "frontmatter": {
        "title": "策略模式"
      }
    },
    {
      "key": "v-5927fbc016bb1",
      "path": "/knowledge/design/%E7%AE%80%E5%8D%95%E5%B7%A5%E5%8E%82%E6%A8%A1%E5%BC%8F.html",
      "lastUpdated": 1574161526000,
      "title": "简单工厂模式",
      "headers": [
        {
          "level": 2,
          "title": "适用场景",
          "slug": "适用场景"
        },
        {
          "level": 2,
          "title": "核心思想",
          "slug": "核心思想"
        },
        {
          "level": 2,
          "title": "举例说明",
          "slug": "举例说明"
        },
        {
          "level": 2,
          "title": "总结",
          "slug": "总结"
        }
      ],
      "frontmatter": {
        "title": "简单工厂模式"
      }
    },
    {
      "key": "v-d7a4905ca843a",
      "path": "/knowledge/design/%E8%A3%85%E9%A5%B0%E8%80%85%E6%A8%A1%E5%BC%8F.html",
      "lastUpdated": 1574245566000,
      "title": "装饰者模式",
      "headers": [
        {
          "level": 2,
          "title": "适用场景",
          "slug": "适用场景"
        },
        {
          "level": 2,
          "title": "核心思想",
          "slug": "核心思想"
        },
        {
          "level": 2,
          "title": "举例说明",
          "slug": "举例说明"
        },
        {
          "level": 2,
          "title": "总结",
          "slug": "总结"
        }
      ],
      "frontmatter": {
        "title": "装饰者模式"
      }
    },
    {
      "key": "v-ab683758f6bb6",
      "path": "/knowledge/design/%E8%BF%AA%E7%B1%B3%E7%89%B9%E6%B3%95%E5%88%99.html",
      "lastUpdated": 1575515744000,
      "title": "迪米特法则",
      "headers": [
        {
          "level": 2,
          "title": "定义",
          "slug": "定义"
        },
        {
          "level": 2,
          "title": "反面例子",
          "slug": "反面例子"
        },
        {
          "level": 2,
          "title": "正面教材",
          "slug": "正面教材"
        },
        {
          "level": 2,
          "title": "总结",
          "slug": "总结"
        }
      ],
      "frontmatter": {
        "title": "迪米特法则"
      }
    },
    {
      "key": "v-25ba05184fc0a",
      "path": "/knowledge/english/read.html",
      "lastUpdated": 1573615737000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-d3da273920d88",
      "path": "/knowledge/english/%E8%8B%B1%E8%AF%AD%E8%AF%AD%E6%B3%95%E6%96%B0%E6%80%9D%E7%BB%B4%E5%88%9D%E7%BA%A7.html",
      "lastUpdated": 1573615737000,
      "title": "英语语法新思维初级教程-走进语法",
      "headers": [
        {
          "level": 2,
          "title": "绪论 名词短语",
          "slug": "绪论-名词短语"
        },
        {
          "level": 3,
          "title": "语言的构造级别",
          "slug": "语言的构造级别"
        },
        {
          "level": 3,
          "title": "名词短语的构造",
          "slug": "名词短语的构造"
        },
        {
          "level": 3,
          "title": "汉语与英语的差别",
          "slug": "汉语与英语的差别"
        },
        {
          "level": 3,
          "title": "英语句子的五成分论",
          "slug": "英语句子的五成分论"
        }
      ],
      "frontmatter": {
        "title": "英语语法新思维初级教程-走进语法"
      }
    },
    {
      "key": "v-1e76bd1a5b234",
      "path": "/knowledge/hard/read.html",
      "lastUpdated": 1571138319000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-d2155372ffbf6",
      "path": "/knowledge/mindmap/read.html",
      "lastUpdated": 1572934205000,
      "title": "介绍",
      "headers": [
        {
          "level": 2,
          "title": "程序员练级攻略",
          "slug": "程序员练级攻略"
        }
      ],
      "frontmatter": {
        "title": "介绍",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-9b407bf7efe95",
      "path": "/knowledge/mindmap/%E5%89%8D%E7%AB%AF%E5%9F%BA%E7%A1%80%E4%B8%8E%E5%BA%95%E5%B1%82%E5%8E%9F%E7%90%86.html",
      "lastUpdated": 1575199968000,
      "title": "前端基础与底层原理",
      "frontmatter": {
        "title": "前端基础与底层原理"
      }
    },
    {
      "key": "v-e69dcc38d046d",
      "path": "/knowledge/mindmap/%E5%A6%82%E4%BD%95%E9%98%85%E8%AF%BB%E4%B8%80%E6%9C%AC%E8%8B%B1%E6%96%87%E4%B9%A6.html",
      "lastUpdated": 1575199968000,
      "title": "如何阅读一本英文书",
      "frontmatter": {
        "title": "如何阅读一本英文书"
      }
    },
    {
      "key": "v-b6256ecaacaf6",
      "path": "/knowledge/mindmap/%E5%AD%A6%E4%B9%A0%E5%92%8C%E9%98%85%E8%AF%BB%E6%BA%90%E7%A0%81.html",
      "lastUpdated": 1575199968000,
      "title": "学习和阅读源码",
      "frontmatter": {
        "title": "学习和阅读源码"
      }
    },
    {
      "key": "v-a5e39c2b2caec",
      "path": "/knowledge/mindmap/%E5%BC%82%E6%AD%A5IO%E4%B8%8ELock_Free.html",
      "lastUpdated": 1575199968000,
      "title": "异步IO与Lock_Free",
      "frontmatter": {
        "title": "异步IO与Lock_Free"
      }
    },
    {
      "key": "v-b6570d814f8e8",
      "path": "/knowledge/mindmap/%E6%AD%A3%E5%BC%8F%E5%85%A5%E9%97%A8.html",
      "lastUpdated": 1575199968000,
      "title": "正式入门",
      "frontmatter": {
        "title": "正式入门"
      }
    },
    {
      "key": "v-8a9a5ce318305",
      "path": "/knowledge/mindmap/%E6%B7%B1%E5%BA%A6%E5%BD%92%E7%BA%B3%E5%92%8C%E5%9D%9A%E6%8C%81%E5%AE%9E%E8%B7%B5.html",
      "lastUpdated": 1575199968000,
      "title": "深度、归纳和坚持实践",
      "frontmatter": {
        "title": "深度、归纳和坚持实践"
      }
    },
    {
      "key": "v-9fed2c1e3940d",
      "path": "/knowledge/mindmap/%E6%BA%90%E5%A4%B4%E5%8E%9F%E7%90%86%E5%92%8C%E7%9F%A5%E8%AF%86%E5%9C%B0%E5%9B%BE.html",
      "lastUpdated": 1575199968000,
      "title": "源头、原理和知识地图",
      "frontmatter": {
        "title": "源头、原理和知识地图"
      }
    },
    {
      "key": "v-5b3afe84cca4",
      "path": "/knowledge/mindmap/%E7%90%86%E8%AE%BA%E5%AD%A6%E7%A7%91.html",
      "lastUpdated": 1575199968000,
      "title": "理论学科",
      "frontmatter": {
        "title": "理论学科"
      }
    },
    {
      "key": "v-1bb09cbbfedc5",
      "path": "/knowledge/mindmap/%E7%A8%8B%E5%BA%8F%E5%91%98%E7%BB%83%E7%BA%A7%E6%94%BB%E7%95%A5%E5%85%A5%E9%97%A8.html",
      "lastUpdated": 1575199968000,
      "title": "程序员练级攻略-入门",
      "frontmatter": {
        "title": "程序员练级攻略-入门"
      }
    },
    {
      "key": "v-7ebf1f828f832",
      "path": "/knowledge/mindmap/%E7%A8%8B%E5%BA%8F%E5%91%98%E7%BB%83%E7%BA%A7%E6%94%BB%E7%95%A5%E5%BC%80%E7%AF%87.html",
      "lastUpdated": 1575199968000,
      "title": "程序员练级攻略-开篇",
      "frontmatter": {
        "title": "程序员练级攻略-开篇"
      }
    },
    {
      "key": "v-97b30daa887cd",
      "path": "/knowledge/mindmap/%E7%A8%8B%E5%BA%8F%E5%91%98%E7%BB%83%E7%BA%A7%E6%94%BB%E7%95%A5%E8%BF%9B%E9%98%B6.html",
      "lastUpdated": 1575199968000,
      "title": "程序员练级攻略-进阶",
      "frontmatter": {
        "title": "程序员练级攻略-进阶"
      }
    },
    {
      "key": "v-fd83c3d277da1",
      "path": "/knowledge/mindmap/%E7%AB%AF%E6%AD%A3%E5%AD%A6%E4%B9%A0%E6%80%81%E5%BA%A6.html",
      "lastUpdated": 1575199968000,
      "title": "端正学习态度",
      "frontmatter": {
        "title": "端正学习态度"
      }
    },
    {
      "key": "v-f150c8daa3744",
      "path": "/knowledge/mindmap/%E7%B3%BB%E7%BB%9F%E5%BA%95%E5%B1%82.html",
      "lastUpdated": 1575199968000,
      "title": "系统底层",
      "frontmatter": {
        "title": "系统底层"
      }
    },
    {
      "key": "v-e7941fde2054c",
      "path": "/knowledge/mindmap/%E7%B3%BB%E7%BB%9F%E7%9F%A5%E8%AF%86.html",
      "lastUpdated": 1575199968000,
      "title": "系统知识",
      "frontmatter": {
        "title": "系统知识"
      }
    },
    {
      "key": "v-aa58ca2d2f9c2",
      "path": "/knowledge/mindmap/%E7%BC%96%E7%A8%8B%E7%B4%A0%E5%85%BB.html",
      "lastUpdated": 1575199968000,
      "title": "编程素养",
      "frontmatter": {
        "title": "编程素养"
      }
    },
    {
      "key": "v-af395b9b38f4b",
      "path": "/knowledge/mindmap/%E7%BC%96%E7%A8%8B%E8%AF%AD%E8%A8%80.html",
      "lastUpdated": 1575199968000,
      "title": "编程语言",
      "frontmatter": {
        "title": "编程语言"
      }
    },
    {
      "key": "v-0b82460256fbb",
      "path": "/knowledge/mindmap/%E8%87%AA%E6%88%91%E7%AE%A1%E7%90%86.html",
      "lastUpdated": 1575199968000,
      "title": "自我管理",
      "frontmatter": {
        "title": "自我管理"
      }
    },
    {
      "key": "v-08186ae15f553",
      "path": "/knowledge/mindmap/%E8%BD%AF%E4%BB%B6%E8%AE%BE%E8%AE%A1.html",
      "lastUpdated": 1575199968000,
      "title": "软件设计",
      "frontmatter": {
        "title": "软件设计"
      }
    },
    {
      "key": "v-1a3badbf0366b",
      "path": "/knowledge/mindmap/%E9%9B%B6%E5%9F%BA%E7%A1%80%E5%90%AF%E8%92%99.html",
      "lastUpdated": 1575199968000,
      "title": "零基础启蒙",
      "frontmatter": {
        "title": "零基础启蒙"
      }
    },
    {
      "key": "v-8a2c930c502da",
      "path": "/knowledge/mindmap/%E9%9D%A2%E5%AF%B9%E6%9E%AF%E7%87%A5%E5%92%8C%E9%87%8F%E5%A4%A7%E7%9A%84%E7%9F%A5%E8%AF%86.html",
      "lastUpdated": 1575199968000,
      "title": "面对枯燥和量大的知识",
      "frontmatter": {
        "title": "面对枯燥和量大的知识"
      }
    },
    {
      "key": "v-c81df4cdf0249",
      "path": "/knowledge/mindmap/%E9%AB%98%E6%95%88%E5%AD%A6%E4%B9%A0.html",
      "lastUpdated": 1575199968000,
      "title": "高效学习",
      "frontmatter": {
        "title": "高效学习"
      }
    },
    {
      "key": "v-754dd37277a88",
      "path": "/knowledge/network/read.html",
      "lastUpdated": 1571138319000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-ed476b7dfbf55",
      "path": "/knowledge/structure/read.html",
      "lastUpdated": 1574335933000,
      "title": "数据结构绪论",
      "headers": [
        {
          "level": 2,
          "title": "数据结构的重要性",
          "slug": "数据结构的重要性"
        },
        {
          "level": 2,
          "title": "什么是数据结构",
          "slug": "什么是数据结构"
        },
        {
          "level": 2,
          "title": "逻辑结构与物理结构",
          "slug": "逻辑结构与物理结构"
        },
        {
          "level": 3,
          "title": "逻辑结构",
          "slug": "逻辑结构"
        },
        {
          "level": 3,
          "title": "物理结构",
          "slug": "物理结构"
        },
        {
          "level": 2,
          "title": "数据类型",
          "slug": "数据类型"
        },
        {
          "level": 2,
          "title": "抽象数据类型",
          "slug": "抽象数据类型"
        }
      ],
      "frontmatter": {
        "title": "数据结构绪论"
      }
    },
    {
      "key": "v-e10d691263b72",
      "path": "/knowledge/structure/%E7%AE%97%E6%B3%95.html",
      "lastUpdated": 1574335933000,
      "title": "算法",
      "headers": [
        {
          "level": 2,
          "title": "算法定义",
          "slug": "算法定义"
        },
        {
          "level": 2,
          "title": "比较两种算法",
          "slug": "比较两种算法"
        },
        {
          "level": 2,
          "title": "算法的特性",
          "slug": "算法的特性"
        },
        {
          "level": 3,
          "title": "输入输出",
          "slug": "输入输出"
        },
        {
          "level": 3,
          "title": "有穷性",
          "slug": "有穷性"
        },
        {
          "level": 3,
          "title": "确定性",
          "slug": "确定性"
        },
        {
          "level": 3,
          "title": "可行性",
          "slug": "可行性"
        },
        {
          "level": 2,
          "title": "算法的设计要求",
          "slug": "算法的设计要求"
        },
        {
          "level": 3,
          "title": "正确性",
          "slug": "正确性"
        },
        {
          "level": 3,
          "title": "可读性",
          "slug": "可读性"
        },
        {
          "level": 3,
          "title": "健壮性",
          "slug": "健壮性"
        },
        {
          "level": 3,
          "title": "时间效率高和存储量低",
          "slug": "时间效率高和存储量低"
        },
        {
          "level": 2,
          "title": "算法效率的度量方法",
          "slug": "算法效率的度量方法"
        },
        {
          "level": 3,
          "title": "事后统计方法",
          "slug": "事后统计方法"
        },
        {
          "level": 3,
          "title": "事前分析估算方法",
          "slug": "事前分析估算方法"
        },
        {
          "level": 2,
          "title": "算法时间的复杂度",
          "slug": "算法时间的复杂度"
        },
        {
          "level": 3,
          "title": "定义",
          "slug": "定义"
        },
        {
          "level": 3,
          "title": "推导大O阶方法",
          "slug": "推导大o阶方法"
        },
        {
          "level": 3,
          "title": "常见的时间复杂度",
          "slug": "常见的时间复杂度"
        },
        {
          "level": 2,
          "title": "最坏情况与平均情况",
          "slug": "最坏情况与平均情况"
        },
        {
          "level": 2,
          "title": "算法空间复杂度",
          "slug": "算法空间复杂度"
        }
      ],
      "frontmatter": {
        "title": "算法"
      }
    },
    {
      "key": "v-b30b578bc457d",
      "path": "/knowledge/structure/%E7%BA%BF%E6%80%A7%E8%A1%A8.html",
      "lastUpdated": 1575025710000,
      "title": "线性表",
      "headers": [
        {
          "level": 2,
          "title": "定义",
          "slug": "定义"
        },
        {
          "level": 2,
          "title": "线性表的操作方法",
          "slug": "线性表的操作方法"
        },
        {
          "level": 2,
          "title": "线性表的顺序存储结构",
          "slug": "线性表的顺序存储结构"
        },
        {
          "level": 3,
          "title": "数组长度与线性表长度的区别",
          "slug": "数组长度与线性表长度的区别"
        },
        {
          "level": 3,
          "title": "地址计算方法",
          "slug": "地址计算方法"
        },
        {
          "level": 2,
          "title": "顺序存储结构的插入与删除",
          "slug": "顺序存储结构的插入与删除"
        },
        {
          "level": 3,
          "title": "获得元素操作",
          "slug": "获得元素操作"
        },
        {
          "level": 3,
          "title": "插入操作",
          "slug": "插入操作"
        },
        {
          "level": 3,
          "title": "删除操作",
          "slug": "删除操作"
        },
        {
          "level": 3,
          "title": "线性表顺序存储结构的优缺点",
          "slug": "线性表顺序存储结构的优缺点"
        },
        {
          "level": 2,
          "title": "线性表的链式存储结构",
          "slug": "线性表的链式存储结构"
        }
      ],
      "frontmatter": {
        "title": "线性表"
      }
    },
    {
      "key": "v-fb0e7fd9d4276",
      "path": "/knowledge/system/read.html",
      "lastUpdated": 1571138319000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-938c096c438e4",
      "path": "/knowledge/web/css.html",
      "lastUpdated": 1566380291000,
      "title": "css",
      "frontmatter": {
        "title": "css",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-3296b442eddc4",
      "path": "/knowledge/web/html-%E4%BA%A4%E5%8F%89%E8%A7%82%E5%AF%9F%E8%80%85.html",
      "lastUpdated": 1573615737000,
      "title": "交叉观察者",
      "headers": [
        {
          "level": 2,
          "title": "交叉观察者是个啥？",
          "slug": "交叉观察者是个啥？"
        },
        {
          "level": 2,
          "title": "基本用法",
          "slug": "基本用法"
        },
        {
          "level": 2,
          "title": "简单例子",
          "slug": "简单例子"
        },
        {
          "level": 2,
          "title": "实际应用",
          "slug": "实际应用"
        },
        {
          "level": 3,
          "title": "图片懒加载",
          "slug": "图片懒加载"
        },
        {
          "level": 3,
          "title": "触底",
          "slug": "触底"
        },
        {
          "level": 2,
          "title": "动画展示",
          "slug": "动画展示"
        },
        {
          "level": 2,
          "title": "总结",
          "slug": "总结"
        }
      ],
      "frontmatter": {
        "title": "交叉观察者"
      }
    },
    {
      "key": "v-f7165c2f953c",
      "path": "/knowledge/web/html-%E6%BB%9A%E5%8A%A8%E6%9D%A1.html",
      "lastUpdated": 1573441509000,
      "title": "滚动条",
      "headers": [
        {
          "level": 2,
          "title": "分类",
          "slug": "分类"
        },
        {
          "level": 2,
          "title": "如何设置全局滚动条高度",
          "slug": "如何设置全局滚动条高度"
        },
        {
          "level": 2,
          "title": "如何指定一个元素显示在视窗",
          "slug": "如何指定一个元素显示在视窗"
        },
        {
          "level": 2,
          "title": "如何设置滚动具有平滑的过渡效果",
          "slug": "如何设置滚动具有平滑的过渡效果"
        },
        {
          "level": 2,
          "title": "其他有趣的知识点",
          "slug": "其他有趣的知识点"
        },
        {
          "level": 3,
          "title": "scrollingElement",
          "slug": "scrollingelement"
        },
        {
          "level": 3,
          "title": "滚动到底部",
          "slug": "滚动到底部"
        },
        {
          "level": 3,
          "title": "解决IOS设备局部滚动不顺畅(粘手)",
          "slug": "解决ios设备局部滚动不顺畅-粘手"
        },
        {
          "level": 3,
          "title": "滚动传播",
          "slug": "滚动传播"
        },
        {
          "level": 3,
          "title": "滚动结束后，强制滚动到指定元素",
          "slug": "滚动结束后，强制滚动到指定元素"
        }
      ],
      "frontmatter": {
        "title": "滚动条"
      }
    },
    {
      "key": "v-a7549a5459992",
      "path": "/knowledge/web/html.html",
      "lastUpdated": 1573441509000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-3f82c8620a0e5",
      "path": "/knowledge/web/javascript.html",
      "lastUpdated": 1566380291000,
      "title": "javascript",
      "frontmatter": {
        "title": "javascript",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-feeba4493b80c",
      "path": "/knowledge/web/react-%E7%BC%96%E7%A0%81%E8%A7%84%E8%8C%83.html",
      "lastUpdated": 1575020017000,
      "title": "编码规范",
      "headers": [
        {
          "level": 2,
          "title": "基础规则",
          "slug": "基础规则"
        },
        {
          "level": 2,
          "title": "文件命名",
          "slug": "文件命名"
        },
        {
          "level": 2,
          "title": "模块化",
          "slug": "模块化"
        },
        {
          "level": 2,
          "title": "组件声明",
          "slug": "组件声明"
        },
        {
          "level": 2,
          "title": "React中的命名",
          "slug": "react中的命名"
        },
        {
          "level": 2,
          "title": "JSX写法注意",
          "slug": "jsx写法注意"
        },
        {
          "level": 3,
          "title": "标签",
          "slug": "标签"
        },
        {
          "level": 3,
          "title": "对齐",
          "slug": "对齐"
        },
        {
          "level": 3,
          "title": "引号",
          "slug": "引号"
        },
        {
          "level": 2,
          "title": "样式写法",
          "slug": "样式写法"
        },
        {
          "level": 2,
          "title": "defaultProps使用静态属性定义",
          "slug": "defaultprops使用静态属性定义"
        },
        {
          "level": 2,
          "title": "key属性设置",
          "slug": "key属性设置"
        },
        {
          "level": 2,
          "title": "为组件绑定事件处理器",
          "slug": "为组件绑定事件处理器"
        },
        {
          "level": 2,
          "title": "State",
          "slug": "state"
        },
        {
          "level": 3,
          "title": "不要直接修改state",
          "slug": "不要直接修改state"
        },
        {
          "level": 3,
          "title": "State的更新可能是异步的",
          "slug": "state的更新可能是异步的"
        },
        {
          "level": 2,
          "title": "组件的代码顺序",
          "slug": "组件的代码顺序"
        },
        {
          "level": 2,
          "title": "使用高阶组件",
          "slug": "使用高阶组件"
        },
        {
          "level": 2,
          "title": "避免不必要render的写法",
          "slug": "避免不必要render的写法"
        },
        {
          "level": 2,
          "title": "状态提升",
          "slug": "状态提升"
        },
        {
          "level": 2,
          "title": "推荐使用Context",
          "slug": "推荐使用context"
        },
        {
          "level": 2,
          "title": "Refs写法",
          "slug": "refs写法"
        },
        {
          "level": 2,
          "title": "路由加载",
          "slug": "路由加载"
        },
        {
          "level": 2,
          "title": "AJAX发起请求的时机",
          "slug": "ajax发起请求的时机"
        }
      ],
      "frontmatter": {
        "title": "编码规范"
      }
    },
    {
      "key": "v-00c86b939c247",
      "path": "/knowledge/web/react.html",
      "lastUpdated": 1571138319000,
      "title": "React",
      "frontmatter": {
        "title": "React",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-985a80dcc8f5f",
      "path": "/knowledge/web/read.html",
      "lastUpdated": 1573441509000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-8968950dbaf74",
      "path": "/knowledge/web/typescript.html",
      "lastUpdated": 1571138319000,
      "title": "TypeScript",
      "frontmatter": {
        "title": "TypeScript",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-20ac61f73401c",
      "path": "/knowledge/web/vue-devtool.html",
      "lastUpdated": 1573441509000,
      "title": "相关工具",
      "headers": [
        {
          "level": 2,
          "title": "Vue Devtools",
          "slug": "vue-devtools"
        }
      ],
      "frontmatter": {
        "title": "相关工具"
      }
    },
    {
      "key": "v-cc231190c573a",
      "path": "/knowledge/web/vue-%E7%BC%96%E7%A0%81%E8%A7%84%E8%8C%83.html",
      "lastUpdated": 1575213132000,
      "title": "编码规范",
      "headers": [
        {
          "level": 2,
          "title": "代码风格",
          "slug": "代码风格"
        },
        {
          "level": 3,
          "title": "缩进",
          "slug": "缩进"
        },
        {
          "level": 3,
          "title": "单行最长限制",
          "slug": "单行最长限制"
        },
        {
          "level": 3,
          "title": "模块书写顺序",
          "slug": "模块书写顺序"
        },
        {
          "level": 2,
          "title": "template部分",
          "slug": "template部分"
        },
        {
          "level": 3,
          "title": "根节点",
          "slug": "根节点"
        },
        {
          "level": 3,
          "title": "标签",
          "slug": "标签"
        },
        {
          "level": 3,
          "title": "属性",
          "slug": "属性"
        },
        {
          "level": 3,
          "title": "指令",
          "slug": "指令"
        },
        {
          "level": 3,
          "title": "插值(Mustache)",
          "slug": "插值-mustache"
        },
        {
          "level": 3,
          "title": "空格",
          "slug": "空格"
        },
        {
          "level": 3,
          "title": "变量",
          "slug": "变量"
        },
        {
          "level": 2,
          "title": "javascript部分",
          "slug": "javascript部分"
        },
        {
          "level": 3,
          "title": "props",
          "slug": "props"
        },
        {
          "level": 3,
          "title": "data",
          "slug": "data"
        },
        {
          "level": 3,
          "title": "变量",
          "slug": "变量-2"
        },
        {
          "level": 3,
          "title": "其他",
          "slug": "其他"
        },
        {
          "level": 2,
          "title": "style部分",
          "slug": "style部分"
        },
        {
          "level": 3,
          "title": "为组件样式设置作用域",
          "slug": "为组件样式设置作用域"
        },
        {
          "level": 2,
          "title": "参考",
          "slug": "参考"
        }
      ],
      "frontmatter": {
        "title": "编码规范"
      }
    },
    {
      "key": "v-5d6f956f9b6e3",
      "path": "/knowledge/web/vue.html",
      "lastUpdated": 1573441509000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-10c4671d9f25f",
      "path": "/knowledge/web/webpack.html",
      "lastUpdated": 1571138319000,
      "title": "Webpack",
      "frontmatter": {
        "title": "Webpack",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-b2cbdc02475ae",
      "path": "/notebook/algorithm/read.html",
      "lastUpdated": 1572934205000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-2d8a7b1d89295",
      "path": "/notebook/algorithm/shuffle.html",
      "lastUpdated": 1572934205000,
      "title": "洗牌",
      "headers": [
        {
          "level": 2,
          "title": "洗牌算法",
          "slug": "洗牌算法"
        }
      ],
      "frontmatter": {
        "title": "洗牌",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-4dca73a362f54",
      "path": "/notebook/cssTricks/behavior.html",
      "lastUpdated": 1572934205000,
      "title": "Behavior Skill",
      "headers": [
        {
          "level": 2,
          "title": "使用overflow-scrolling支持弹性滚动",
          "slug": "使用overflow-scrolling支持弹性滚动"
        },
        {
          "level": 2,
          "title": "使用transform启动GPU硬件加速",
          "slug": "使用transform启动gpu硬件加速"
        },
        {
          "level": 2,
          "title": "使用attr()抓取data-*",
          "slug": "使用attr-抓取data"
        },
        {
          "level": 2,
          "title": "使用:valid和:invalid校验表单",
          "slug": "使用-valid和-invalid校验表单"
        },
        {
          "level": 2,
          "title": "使用pointer-events禁用事件触发",
          "slug": "使用pointer-events禁用事件触发"
        },
        {
          "level": 2,
          "title": "使用+或~美化选项框",
          "slug": "使用-或-美化选项框"
        },
        {
          "level": 2,
          "title": "使用:focus-within分发冒泡响应",
          "slug": "使用-focus-within分发冒泡响应"
        },
        {
          "level": 2,
          "title": "使用:hover描绘鼠标跟随",
          "slug": "使用-hover描绘鼠标跟随"
        },
        {
          "level": 2,
          "title": "使用max-height切换自动高度",
          "slug": "使用max-height切换自动高度"
        },
        {
          "level": 2,
          "title": "使用transform模拟视差滚动",
          "slug": "使用transform模拟视差滚动"
        },
        {
          "level": 2,
          "title": "使用animation-delay保留动画起始帧",
          "slug": "使用animation-delay保留动画起始帧"
        },
        {
          "level": 2,
          "title": "使用resize拉伸分栏",
          "slug": "使用resize拉伸分栏"
        }
      ],
      "frontmatter": {
        "title": "Behavior Skill",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-3d332ac11f48",
      "path": "/notebook/cssTricks/color.html",
      "lastUpdated": 1572934205000,
      "title": "Color Skill",
      "headers": [
        {
          "level": 2,
          "title": "使用color改变边框颜色",
          "slug": "使用color改变边框颜色"
        },
        {
          "level": 2,
          "title": "使用filter开启悼念模式",
          "slug": "使用filter开启悼念模式"
        },
        {
          "level": 2,
          "title": "使用::selection改变文本选择颜色",
          "slug": "使用-selection改变文本选择颜色"
        },
        {
          "level": 2,
          "title": "使用linear-gradient控制背景渐变",
          "slug": "使用linear-gradient控制背景渐变"
        },
        {
          "level": 2,
          "title": "使用linear-gradient控制文本渐变",
          "slug": "使用linear-gradient控制文本渐变"
        },
        {
          "level": 2,
          "title": "使用caret-color改变光标颜色",
          "slug": "使用caret-color改变光标颜色"
        },
        {
          "level": 2,
          "title": "使用:scrollbar改变滚动条样式",
          "slug": "使用-scrollbar改变滚动条样式"
        },
        {
          "level": 2,
          "title": "使用filter模拟Instagram滤镜",
          "slug": "使用filter模拟instagram滤镜"
        }
      ],
      "frontmatter": {
        "title": "Color Skill",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-d5ff642b40079",
      "path": "/notebook/cssTricks/component.html",
      "lastUpdated": 1572934205000,
      "title": "Compoent Skill",
      "headers": [
        {
          "level": 2,
          "title": "迭代计数器",
          "slug": "迭代计数器"
        },
        {
          "level": 2,
          "title": "下划线跟随导航栏",
          "slug": "下划线跟随导航栏"
        },
        {
          "level": 2,
          "title": "气泡背景墙",
          "slug": "气泡背景墙"
        },
        {
          "level": 2,
          "title": "滚动指示器",
          "slug": "滚动指示器"
        },
        {
          "level": 2,
          "title": "故障文本",
          "slug": "故障文本"
        },
        {
          "level": 2,
          "title": "换色器",
          "slug": "换色器"
        },
        {
          "level": 2,
          "title": "状态悬浮球",
          "slug": "状态悬浮球"
        },
        {
          "level": 2,
          "title": "粘粘球",
          "slug": "粘粘球"
        },
        {
          "level": 2,
          "title": "商城票券",
          "slug": "商城票券"
        },
        {
          "level": 2,
          "title": "倒影加载条",
          "slug": "倒影加载条"
        },
        {
          "level": 2,
          "title": "三维立方体",
          "slug": "三维立方体"
        },
        {
          "level": 2,
          "title": "动态边框",
          "slug": "动态边框"
        },
        {
          "level": 2,
          "title": "标签页",
          "slug": "标签页"
        },
        {
          "level": 2,
          "title": "标签导航栏",
          "slug": "标签导航栏"
        },
        {
          "level": 2,
          "title": "折叠面板",
          "slug": "折叠面板"
        },
        {
          "level": 2,
          "title": "星级评分",
          "slug": "星级评分"
        },
        {
          "level": 2,
          "title": "加载指示器",
          "slug": "加载指示器"
        },
        {
          "level": 2,
          "title": "自适应相册",
          "slug": "自适应相册"
        },
        {
          "level": 2,
          "title": "圆角进度条",
          "slug": "圆角进度条"
        },
        {
          "level": 2,
          "title": "螺纹进度条",
          "slug": "螺纹进度条"
        },
        {
          "level": 2,
          "title": "立体按钮",
          "slug": "立体按钮"
        },
        {
          "level": 2,
          "title": "混沌加载器",
          "slug": "混沌加载器"
        },
        {
          "level": 2,
          "title": "蛇形边框",
          "slug": "蛇形边框"
        },
        {
          "level": 2,
          "title": "自动打字",
          "slug": "自动打字"
        }
      ],
      "frontmatter": {
        "title": "Compoent Skill",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-9606a9fd0c61e",
      "path": "/notebook/cssTricks/figure.html",
      "lastUpdated": 1572934205000,
      "title": "Figure Skill",
      "headers": [
        {
          "level": 2,
          "title": "使用div描绘各种图形",
          "slug": "使用div描绘各种图形"
        },
        {
          "level": 2,
          "title": "使用mask雕刻镂空背景",
          "slug": "使用mask雕刻镂空背景"
        },
        {
          "level": 2,
          "title": "使用linear-gradient描绘波浪线",
          "slug": "使用linear-gradient描绘波浪线"
        },
        {
          "level": 2,
          "title": "使用linear-gradient描绘彩带",
          "slug": "使用linear-gradient描绘彩带"
        },
        {
          "level": 2,
          "title": "使用conic-gradient描绘饼图",
          "slug": "使用conic-gradient描绘饼图"
        },
        {
          "level": 2,
          "title": "使用linear-gradient描绘方格背景",
          "slug": "使用linear-gradient描绘方格背景"
        },
        {
          "level": 2,
          "title": "使用box-shadow描绘单侧投影",
          "slug": "使用box-shadow描绘单侧投影"
        },
        {
          "level": 2,
          "title": "使用filter描绘头像彩色阴影",
          "slug": "使用filter描绘头像彩色阴影"
        },
        {
          "level": 2,
          "title": "使用box-shadow裁剪图像",
          "slug": "使用box-shadow裁剪图像"
        },
        {
          "level": 2,
          "title": "使用outline描绘内边框",
          "slug": "使用outline描绘内边框"
        }
      ],
      "frontmatter": {
        "title": "Figure Skill",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-25412f0b2efa8",
      "path": "/notebook/cssTricks/layout.html",
      "lastUpdated": 1574220385000,
      "title": "Layout Skill",
      "headers": [
        {
          "level": 2,
          "title": "使用vw定制rem自适应布局",
          "slug": "使用vw定制rem自适应布局"
        },
        {
          "level": 2,
          "title": "使用:nth-child()选择指定元素",
          "slug": "使用-nth-child-选择指定元素"
        },
        {
          "level": 2,
          "title": "使用writing-mode排版竖文",
          "slug": "使用writing-mode排版竖文"
        },
        {
          "level": 2,
          "title": "使用text-align-last对齐两端文本",
          "slug": "使用text-align-last对齐两端文本"
        },
        {
          "level": 2,
          "title": "使用:not()去除无用属性",
          "slug": "使用-not-去除无用属性"
        },
        {
          "level": 2,
          "title": "使用object-fit规定图像尺寸",
          "slug": "使用object-fit规定图像尺寸"
        },
        {
          "level": 2,
          "title": "使用overflow-x排版横向列表",
          "slug": "使用overflow-x排版横向列表"
        },
        {
          "level": 2,
          "title": "使用text-overflow控制文本溢出",
          "slug": "使用text-overflow控制文本溢出"
        },
        {
          "level": 2,
          "title": "使用transform描绘1px边框",
          "slug": "使用transform描绘1px边框"
        },
        {
          "level": 2,
          "title": "使用transform翻转内容",
          "slug": "使用transform翻转内容"
        },
        {
          "level": 2,
          "title": "使用letter-spacing排版倒序文本",
          "slug": "使用letter-spacing排版倒序文本"
        },
        {
          "level": 2,
          "title": "使用margin-left排版左重右轻列表",
          "slug": "使用margin-left排版左重右轻列表"
        },
        {
          "level": 2,
          "title": "使用padding-top实现图片宽高相同",
          "slug": "使用padding-top实现图片宽高相同"
        }
      ],
      "frontmatter": {
        "title": "Layout Skill",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-305f7bbcfe67e",
      "path": "/notebook/cssTricks/read.html",
      "lastUpdated": 1571651981000,
      "title": "介绍",
      "headers": [
        {
          "level": 2,
          "title": "收集常用的CSS方法",
          "slug": "收集常用的css方法"
        }
      ],
      "frontmatter": {
        "title": "介绍",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-c49be195c8082",
      "path": "/notebook/difters/read.html",
      "lastUpdated": 1575020017000,
      "title": "介绍",
      "headers": [
        {
          "level": 2,
          "title": "北漂日志",
          "slug": "北漂日志"
        }
      ],
      "frontmatter": {
        "title": "介绍",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-9b7bcbf86531a",
      "path": "/notebook/difters/%E6%89%AC%E5%B8%86%E8%B5%B7%E8%88%AA.html",
      "lastUpdated": 1572934205000,
      "title": "扬帆起航",
      "headers": [
        {
          "level": 2,
          "title": "扬帆起航",
          "slug": "扬帆起航"
        }
      ],
      "frontmatter": {
        "title": "扬帆起航",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-b38927f8be867",
      "path": "/notebook/difters/%E7%AC%AC%E4%B8%80%E7%AB%A0.html",
      "lastUpdated": 1572934205000,
      "title": "临行前夕",
      "headers": [
        {
          "level": 2,
          "title": "临行前夕",
          "slug": "临行前夕"
        }
      ],
      "frontmatter": {
        "title": "临行前夕",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-ffb69f6fcbcd4",
      "path": "/notebook/jsTricks/array.html",
      "lastUpdated": 1572584172000,
      "title": "Array Skill",
      "headers": [
        {
          "level": 2,
          "title": "克隆数组",
          "slug": "克隆数组"
        },
        {
          "level": 2,
          "title": "类数组对象转为数组",
          "slug": "类数组对象转为数组"
        },
        {
          "level": 2,
          "title": "合并数组",
          "slug": "合并数组"
        },
        {
          "level": 2,
          "title": "去重数组",
          "slug": "去重数组"
        },
        {
          "level": 2,
          "title": "混淆数组",
          "slug": "混淆数组"
        },
        {
          "level": 2,
          "title": "清空数组",
          "slug": "清空数组"
        },
        {
          "level": 2,
          "title": "截断数组",
          "slug": "截断数组"
        },
        {
          "level": 2,
          "title": "交换赋值",
          "slug": "交换赋值"
        },
        {
          "level": 2,
          "title": "过滤假值",
          "slug": "过滤假值"
        },
        {
          "level": 2,
          "title": "异步累计",
          "slug": "异步累计"
        },
        {
          "level": 2,
          "title": "数组首部插入成员",
          "slug": "数组首部插入成员"
        },
        {
          "level": 2,
          "title": "数组尾部插入成员",
          "slug": "数组尾部插入成员"
        },
        {
          "level": 2,
          "title": "统计数组成员个数",
          "slug": "统计数组成员个数"
        },
        {
          "level": 2,
          "title": "解构数组成员嵌套",
          "slug": "解构数组成员嵌套"
        },
        {
          "level": 2,
          "title": "解构数组成员别名",
          "slug": "解构数组成员别名"
        },
        {
          "level": 2,
          "title": "解构数组成员默认值",
          "slug": "解构数组成员默认值"
        },
        {
          "level": 2,
          "title": "获取随机数组成员",
          "slug": "获取随机数组成员"
        },
        {
          "level": 2,
          "title": "创建指定长度数组",
          "slug": "创建指定长度数组"
        },
        {
          "level": 2,
          "title": "创建指定长度且值相等的数组",
          "slug": "创建指定长度且值相等的数组"
        },
        {
          "level": 2,
          "title": "reduce代替map和filter",
          "slug": "reduce代替map和filter"
        }
      ],
      "frontmatter": {
        "title": "Array Skill",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-2c6d404370994",
      "path": "/notebook/jsTricks/boolean.html",
      "lastUpdated": 1571138319000,
      "title": "Boolean Skill",
      "headers": [
        {
          "level": 2,
          "title": "短路运算符",
          "slug": "短路运算符"
        },
        {
          "level": 2,
          "title": "判断数据类型",
          "slug": "判断数据类型"
        },
        {
          "level": 2,
          "title": "是否为空数组",
          "slug": "是否为空数组"
        },
        {
          "level": 2,
          "title": "是否为空对象",
          "slug": "是否为空对象"
        },
        {
          "level": 2,
          "title": "满足条件时执行",
          "slug": "满足条件时执行"
        },
        {
          "level": 2,
          "title": "为非假值时执行",
          "slug": "为非假值时执行"
        },
        {
          "level": 2,
          "title": "数组不为空时执行",
          "slug": "数组不为空时执行"
        },
        {
          "level": 2,
          "title": "对象不为空时执行",
          "slug": "对象不为空时执行"
        },
        {
          "level": 2,
          "title": "函数退出代替条件分支退出",
          "slug": "函数退出代替条件分支退出"
        },
        {
          "level": 2,
          "title": "switch/case使用区间",
          "slug": "switch-case使用区间"
        }
      ],
      "frontmatter": {
        "title": "Boolean Skill",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-6e851981ec7af",
      "path": "/notebook/jsTricks/code.html",
      "lastUpdated": 1573206838000,
      "title": "代码片段",
      "headers": [
        {
          "level": 2,
          "title": "简易轮询",
          "slug": "简易轮询"
        }
      ],
      "frontmatter": {
        "title": "代码片段",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-654c8668c1639",
      "path": "/notebook/jsTricks/dom.html",
      "lastUpdated": 1571138319000,
      "title": "Dom Skill",
      "headers": [
        {
          "level": 2,
          "title": "显示全部DOM边框",
          "slug": "显示全部dom边框"
        },
        {
          "level": 2,
          "title": "自适应网页",
          "slug": "自适应网页"
        },
        {
          "level": 2,
          "title": "过滤XSS",
          "slug": "过滤xss"
        },
        {
          "level": 2,
          "title": "存取LocalStorage",
          "slug": "存取localstorage"
        }
      ],
      "frontmatter": {
        "title": "Dom Skill",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-b8495581e8ce2",
      "path": "/notebook/jsTricks/function.html",
      "lastUpdated": 1571138319000,
      "title": "Function Skill",
      "headers": [
        {
          "level": 2,
          "title": "函数自执行",
          "slug": "函数自执行"
        },
        {
          "level": 2,
          "title": "隐式返回值",
          "slug": "隐式返回值"
        },
        {
          "level": 2,
          "title": "一次性函数",
          "slug": "一次性函数"
        },
        {
          "level": 2,
          "title": "惰性载入函数",
          "slug": "惰性载入函数"
        },
        {
          "level": 2,
          "title": "检测非空参数",
          "slug": "检测非空参数"
        },
        {
          "level": 2,
          "title": "字符串创建函数",
          "slug": "字符串创建函数"
        },
        {
          "level": 2,
          "title": "优雅处理错误信息",
          "slug": "优雅处理错误信息"
        },
        {
          "level": 2,
          "title": "优雅处理Async/Await参数",
          "slug": "优雅处理async-await参数"
        },
        {
          "level": 2,
          "title": "优雅处理多个函数返回值",
          "slug": "优雅处理多个函数返回值"
        }
      ],
      "frontmatter": {
        "title": "Function Skill",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-528eb80cc7979",
      "path": "/notebook/jsTricks/number.html",
      "lastUpdated": 1571637335000,
      "title": "Number Skill",
      "headers": [
        {
          "level": 2,
          "title": "取整",
          "slug": "取整"
        },
        {
          "level": 2,
          "title": "补零",
          "slug": "补零"
        },
        {
          "level": 2,
          "title": "转数值",
          "slug": "转数值"
        },
        {
          "level": 2,
          "title": "时间戳",
          "slug": "时间戳"
        },
        {
          "level": 2,
          "title": "精确小数",
          "slug": "精确小数"
        },
        {
          "level": 2,
          "title": "判断奇偶",
          "slug": "判断奇偶"
        },
        {
          "level": 2,
          "title": "取最小最大值",
          "slug": "取最小最大值"
        },
        {
          "level": 2,
          "title": "生成范围随机数",
          "slug": "生成范围随机数"
        },
        {
          "level": 2,
          "title": "判断NaN",
          "slug": "判断nan"
        }
      ],
      "frontmatter": {
        "title": "Number Skill",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-dbaf43818b805",
      "path": "/notebook/jsTricks/object.html",
      "lastUpdated": 1571138319000,
      "title": "Object Skill",
      "headers": [
        {
          "level": 2,
          "title": "克隆对象",
          "slug": "克隆对象"
        },
        {
          "level": 2,
          "title": "合并对象",
          "slug": "合并对象"
        },
        {
          "level": 2,
          "title": "对象字面量",
          "slug": "对象字面量"
        },
        {
          "level": 2,
          "title": "对象变量属性",
          "slug": "对象变量属性"
        },
        {
          "level": 2,
          "title": "创建纯空对象",
          "slug": "创建纯空对象"
        },
        {
          "level": 2,
          "title": "删除对象无用属性",
          "slug": "删除对象无用属性"
        },
        {
          "level": 2,
          "title": "解构对象属性嵌套",
          "slug": "解构对象属性嵌套"
        },
        {
          "level": 2,
          "title": "解构对象属性别名",
          "slug": "解构对象属性别名"
        }
      ],
      "frontmatter": {
        "title": "Object Skill",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-e8ccecb36fe59",
      "path": "/notebook/jsTricks/read.html",
      "lastUpdated": 1573032621000,
      "title": "介绍",
      "headers": [
        {
          "level": 2,
          "title": "收集常用的JS方法",
          "slug": "收集常用的js方法"
        }
      ],
      "frontmatter": {
        "title": "介绍",
        "sidebarDepth": 0
      }
    },
    {
      "key": "v-1a11161138958",
      "path": "/notebook/jsTricks/string.html",
      "lastUpdated": 1571138319000,
      "title": "String Skill",
      "headers": [
        {
          "level": 2,
          "title": "时间大小对比",
          "slug": "时间大小对比"
        },
        {
          "level": 2,
          "title": "格式化金钱",
          "slug": "格式化金钱"
        },
        {
          "level": 2,
          "title": "生成随机ID",
          "slug": "生成随机id"
        },
        {
          "level": 2,
          "title": "生成随机HEX色值",
          "slug": "生成随机hex色值"
        },
        {
          "level": 2,
          "title": "生成星级评分",
          "slug": "生成星级评分"
        },
        {
          "level": 2,
          "title": "操作URL查询参数",
          "slug": "操作url查询参数"
        }
      ],
      "frontmatter": {
        "title": "String Skill",
        "sidebarDepth": 2
      }
    },
    {
      "key": "v-df7612dafaaf6",
      "path": "/notebook/school/css.html",
      "lastUpdated": 1571138319000,
      "title": "CSS",
      "headers": [
        {
          "level": 2,
          "title": "基础知识",
          "slug": "基础知识"
        },
        {
          "level": 3,
          "title": "盒子模型以及区别和设置",
          "slug": "盒子模型以及区别和设置"
        },
        {
          "level": 3,
          "title": "Position属性值以及相对于谁定位",
          "slug": "position属性值以及相对于谁定位"
        },
        {
          "level": 3,
          "title": "Margin50%是相对于宽还是高",
          "slug": "margin50-是相对于宽还是高"
        },
        {
          "level": 3,
          "title": "CSS3的新特性",
          "slug": "css3的新特性"
        },
        {
          "level": 3,
          "title": "Float布局对后续元素的影响",
          "slug": "float布局对后续元素的影响"
        },
        {
          "level": 3,
          "title": "BFC",
          "slug": "bfc"
        },
        {
          "level": 3,
          "title": "CSS中的伪类before和after",
          "slug": "css中的伪类before和after"
        },
        {
          "level": 3,
          "title": "Display的属性值",
          "slug": "display的属性值"
        },
        {
          "level": 3,
          "title": "Outline和Border的区别",
          "slug": "outline和border的区别"
        },
        {
          "level": 3,
          "title": "CSS单位,rem,em,px的区别,及计算规则",
          "slug": "css单位-rem-em-px的区别-及计算规则"
        },
        {
          "level": 3,
          "title": "块状元素与内联元素的区别",
          "slug": "块状元素与内联元素的区别"
        },
        {
          "level": 3,
          "title": "选择器优先级",
          "slug": "选择器优先级"
        },
        {
          "level": 3,
          "title": "Margin重叠",
          "slug": "margin重叠"
        },
        {
          "level": 3,
          "title": "CSS控制字体粗细,什么属性让字体更清晰",
          "slug": "css控制字体粗细-什么属性让字体更清晰"
        },
        {
          "level": 2,
          "title": "布局相关",
          "slug": "布局相关"
        },
        {
          "level": 3,
          "title": "CSS水平垂直居中",
          "slug": "css水平垂直居中"
        },
        {
          "level": 3,
          "title": "圣杯布局，双飞翼布局",
          "slug": "圣杯布局，双飞翼布局"
        },
        {
          "level": 2,
          "title": "动画",
          "slug": "动画"
        },
        {
          "level": 3,
          "title": "Transform,Transtion和animation的区别",
          "slug": "transform-transtion和animation的区别"
        },
        {
          "level": 3,
          "title": "Transform优化原理和属性",
          "slug": "transform优化原理和属性"
        },
        {
          "level": 3,
          "title": "Javascript动画和CSS动画的区别",
          "slug": "javascript动画和css动画的区别"
        },
        {
          "level": 3,
          "title": "CSS动画如何避免重排",
          "slug": "css动画如何避免重排"
        },
        {
          "level": 3,
          "title": "动画优化,为什么transform比margin的性能好",
          "slug": "动画优化-为什么transform比margin的性能好"
        },
        {
          "level": 2,
          "title": "自适应和响应式",
          "slug": "自适应和响应式"
        },
        {
          "level": 3,
          "title": "响应式网页设计",
          "slug": "响应式网页设计"
        },
        {
          "level": 3,
          "title": "Media query",
          "slug": "media-query"
        },
        {
          "level": 3,
          "title": "Bootstrap布局,如何实现响应式布局",
          "slug": "bootstrap布局-如何实现响应式布局"
        },
        {
          "level": 3,
          "title": "手机端不同屏幕如何适配",
          "slug": "手机端不同屏幕如何适配"
        },
        {
          "level": 2,
          "title": "CSS技巧及应用",
          "slug": "css技巧及应用"
        },
        {
          "level": 3,
          "title": "CSS画一个三角形和圆",
          "slug": "css画一个三角形和圆"
        },
        {
          "level": 3,
          "title": "CSS实现loading",
          "slug": "css实现loading"
        },
        {
          "level": 3,
          "title": "实现progress进度条",
          "slug": "实现progress进度条"
        }
      ]
    },
    {
      "key": "v-7beb57f8b4b",
      "path": "/notebook/school/database.html",
      "lastUpdated": 1571138319000,
      "title": "数据库",
      "headers": [
        {
          "level": 2,
          "title": "MySQL",
          "slug": "mysql"
        },
        {
          "level": 3,
          "title": "mysql数据库引擎和区别",
          "slug": "mysql数据库引擎和区别"
        }
      ]
    },
    {
      "key": "v-e172a58122a74",
      "path": "/notebook/school/freamwork.html",
      "lastUpdated": 1571138319000,
      "title": "JS框架和库",
      "headers": [
        {
          "level": 2,
          "title": "jQuery",
          "slug": "jquery"
        },
        {
          "level": 3,
          "title": "jQuery的优点和缺点",
          "slug": "jquery的优点和缺点"
        },
        {
          "level": 3,
          "title": "写一个类，实现jQuery",
          "slug": "写一个类，实现jquery"
        },
        {
          "level": 3,
          "title": "链式调用的原理",
          "slug": "链式调用的原理"
        },
        {
          "level": 3,
          "title": "$.()和onload的区别",
          "slug": "和onload的区别"
        },
        {
          "level": 3,
          "title": "$.()和onload的区",
          "slug": "和onload的区"
        },
        {
          "level": 3,
          "title": "jQuery与vue区别",
          "slug": "jquery与vue区别"
        },
        {
          "level": 2,
          "title": "Vue",
          "slug": "vue"
        },
        {
          "level": 3,
          "title": "Vue特点",
          "slug": "vue特点"
        },
        {
          "level": 3,
          "title": "组件通信",
          "slug": "组件通信"
        },
        {
          "level": 3,
          "title": "生命周期函数,各个阶段做了什么,应用场景",
          "slug": "生命周期函数-各个阶段做了什么-应用场景"
        },
        {
          "level": 3,
          "title": "双向绑定的实现原理，单向绑定",
          "slug": "双向绑定的实现原理，单向绑定"
        },
        {
          "level": 3,
          "title": "父子组件通信的实现原理",
          "slug": "父子组件通信的实现原理"
        },
        {
          "level": 3,
          "title": "的钩子函数，created和mounted的区别",
          "slug": "的钩子函数，created和mounted的区别"
        },
        {
          "level": 3,
          "title": "虚拟dom，为什么需要虚拟dom，为什么可以加速渲染",
          "slug": "虚拟dom，为什么需要虚拟dom，为什么可以加速渲染"
        },
        {
          "level": 3,
          "title": "Visual dom相关",
          "slug": "visual-dom相关"
        },
        {
          "level": 3,
          "title": "Vuex的理解",
          "slug": "vuex的理解"
        },
        {
          "level": 3,
          "title": "Vue渲染页面的key是什么意思，有什么作用，不设置会如何",
          "slug": "vue渲染页面的key是什么意思，有什么作用，不设置会如何"
        },
        {
          "level": 3,
          "title": "Vue中初始化了VM.date(赋初值),会不会立即渲染到页面，为什么",
          "slug": "vue中初始化了vm-date-赋初值-会不会立即渲染到页面，为什么"
        },
        {
          "level": 3,
          "title": "VUE-router的实现原理",
          "slug": "vue-router的实现原理"
        },
        {
          "level": 3,
          "title": "Vuex中action和mutation区别",
          "slug": "vuex中action和mutation区别"
        },
        {
          "level": 3,
          "title": "Vue单页面应用spa",
          "slug": "vue单页面应用spa"
        }
      ]
    },
    {
      "key": "v-012fc8ca43208",
      "path": "/notebook/school/html.html",
      "lastUpdated": 1571138319000,
      "title": "HTML",
      "headers": [
        {
          "level": 2,
          "title": "基础知识",
          "slug": "基础知识"
        },
        {
          "level": 3,
          "title": "meta标签",
          "slug": "meta标签"
        },
        {
          "level": 3,
          "title": "HTML5的新特征",
          "slug": "html5的新特征"
        },
        {
          "level": 3,
          "title": "script标签的async和defer属性",
          "slug": "script标签的async和defer属性"
        },
        {
          "level": 3,
          "title": "当前的主流浏览器及其内核介绍",
          "slug": "当前的主流浏览器及其内核介绍"
        },
        {
          "level": 3,
          "title": "url从输入到页面渲染发生了什么",
          "slug": "url从输入到页面渲染发生了什么"
        },
        {
          "level": 3,
          "title": "如何延迟HTML文档中的外部脚本文件的加载",
          "slug": "如何延迟html文档中的外部脚本文件的加载"
        },
        {
          "level": 3,
          "title": "WebSocket",
          "slug": "websocket"
        },
        {
          "level": 2,
          "title": "HTML5缓存",
          "slug": "html5缓存"
        },
        {
          "level": 3,
          "title": "浏览器缓存方式",
          "slug": "浏览器缓存方式"
        },
        {
          "level": 3,
          "title": "设置cookie保存时间",
          "slug": "设置cookie保存时间"
        },
        {
          "level": 2,
          "title": "浏览器安全",
          "slug": "浏览器安全"
        },
        {
          "level": 3,
          "title": "XSS和CSRF",
          "slug": "xss和csrf"
        },
        {
          "level": 3,
          "title": "Token",
          "slug": "token"
        },
        {
          "level": 2,
          "title": "性能优化",
          "slug": "性能优化"
        },
        {
          "level": 3,
          "title": "前端性能优化方案及原理",
          "slug": "前端性能优化方案及原理"
        },
        {
          "level": 3,
          "title": "CDN",
          "slug": "cdn"
        },
        {
          "level": 3,
          "title": "图片多了如何优化",
          "slug": "图片多了如何优化"
        }
      ]
    },
    {
      "key": "v-e9d9c073043e4",
      "path": "/notebook/school/javascript.html",
      "lastUpdated": 1571138319000,
      "title": "JAVASCRIPT",
      "headers": [
        {
          "level": 2,
          "title": "JS底层原理",
          "slug": "js底层原理"
        },
        {
          "level": 3,
          "title": "JS是如何实现继承,作用域链,作用域,变量提升",
          "slug": "js是如何实现继承-作用域链-作用域-变量提升"
        },
        {
          "level": 3,
          "title": "NEW的过程做了什么",
          "slug": "new的过程做了什么"
        },
        {
          "level": 3,
          "title": "JS原型链的理解,prototype和_proto_的区别",
          "slug": "js原型链的理解-prototype和-proto-的区别"
        },
        {
          "level": 3,
          "title": "JS的基础数据类型和引用数据类型有哪些及其区别",
          "slug": "js的基础数据类型和引用数据类型有哪些及其区别"
        },
        {
          "level": 3,
          "title": "什么是闭包,优缺点,特征,执行过程以及应用",
          "slug": "什么是闭包-优缺点-特征-执行过程以及应用"
        },
        {
          "level": 3,
          "title": "JS垃圾回收机制",
          "slug": "js垃圾回收机制"
        },
        {
          "level": 3,
          "title": "JS继承的多种方式,组合继承的缺点",
          "slug": "js继承的多种方式-组合继承的缺点"
        },
        {
          "level": 3,
          "title": "异步机制,宏任务,微任务,异步完成后的才执行的函数应该放在哪里",
          "slug": "异步机制-宏任务-微任务-异步完成后的才执行的函数应该放在哪里"
        },
        {
          "level": 3,
          "title": "内存泄露及销毁",
          "slug": "内存泄露及销毁"
        },
        {
          "level": 3,
          "title": "作用域链",
          "slug": "作用域链"
        },
        {
          "level": 3,
          "title": "闭包的this指向谁",
          "slug": "闭包的this指向谁"
        },
        {
          "level": 3,
          "title": "JS内存分配",
          "slug": "js内存分配"
        },
        {
          "level": 3,
          "title": "JS运行环境",
          "slug": "js运行环境"
        },
        {
          "level": 3,
          "title": "一个函数new一下和直接调用的区别",
          "slug": "一个函数new一下和直接调用的区别"
        },
        {
          "level": 2,
          "title": "基础知识",
          "slug": "基础知识"
        },
        {
          "level": 3,
          "title": "Object.prototype.toString与Object.toString的区别,直接toString有什么缺点",
          "slug": "object-prototype-tostring与object-tostring的区别-直接tostring有什么缺点"
        },
        {
          "level": 3,
          "title": "设计模式",
          "slug": "设计模式"
        },
        {
          "level": 3,
          "title": "Typeof判断类型有哪些",
          "slug": "typeof判断类型有哪些"
        },
        {
          "level": 3,
          "title": "正则表达式",
          "slug": "正则表达式"
        },
        {
          "level": 3,
          "title": "MVVM,MVP,MVC的区别，MVVM框架有哪些",
          "slug": "mvvm-mvp-mvc的区别，mvvm框架有哪些"
        },
        {
          "level": 3,
          "title": "变量类型隐式转换规则",
          "slug": "变量类型隐式转换规则"
        },
        {
          "level": 3,
          "title": "事件委托,事件冒泡以及添加事件的方式和区别",
          "slug": "事件委托-事件冒泡以及添加事件的方式和区别"
        },
        {
          "level": 3,
          "title": "嵌套的div,如何确定鼠标点击的是哪个div",
          "slug": "嵌套的div-如何确定鼠标点击的是哪个div"
        },
        {
          "level": 3,
          "title": "获取元素的方式",
          "slug": "获取元素的方式"
        },
        {
          "level": 3,
          "title": "为什么JS是单线程的,多线程有什么问题",
          "slug": "为什么js是单线程的-多线程有什么问题"
        },
        {
          "level": 3,
          "title": "防抖节流",
          "slug": "防抖节流"
        },
        {
          "level": 3,
          "title": "原生js如何实现路由",
          "slug": "原生js如何实现路由"
        },
        {
          "level": 3,
          "title": "setTimeout和Promise区别,谁先执行,setTimeout异步执行原理",
          "slug": "settimeout和promise区别-谁先执行-settimeout异步执行原理"
        },
        {
          "level": 3,
          "title": "如何理解前后端分离的开发理念",
          "slug": "如何理解前后端分离的开发理念"
        },
        {
          "level": 3,
          "title": "如何理解JS以及PY这类脚本语言的概念，他们和java和c++有什么不同",
          "slug": "如何理解js以及py这类脚本语言的概念，他们和java和c-有什么不同"
        },
        {
          "level": 3,
          "title": "强类型与弱类型语言的区别",
          "slug": "强类型与弱类型语言的区别"
        },
        {
          "level": 3,
          "title": "JavaScript中获取dom和创建结点的方法",
          "slug": "javascript中获取dom和创建结点的方法"
        },
        {
          "level": 2,
          "title": "ES6语法相关",
          "slug": "es6语法相关"
        },
        {
          "level": 3,
          "title": "ES6语法,特征,新增的数据类型",
          "slug": "es6语法-特征-新增的数据类型"
        },
        {
          "level": 3,
          "title": "Promise是什么?实现一个,状态如何转换",
          "slug": "promise是什么-实现一个-状态如何转换"
        },
        {
          "level": 3,
          "title": "Class继承",
          "slug": "class继承"
        },
        {
          "level": 3,
          "title": "let,const,var的区别",
          "slug": "let-const-var的区别"
        },
        {
          "level": 3,
          "title": "Object.assign的用法",
          "slug": "object-assign的用法"
        },
        {
          "level": 3,
          "title": "箭头函数和普通函数this区别",
          "slug": "箭头函数和普通函数this区别"
        },
        {
          "level": 3,
          "title": "数组的常用方法",
          "slug": "数组的常用方法"
        },
        {
          "level": 3,
          "title": "Promise实现串行调用，判断多个图片异步加载完成",
          "slug": "promise实现串行调用，判断多个图片异步加载完成"
        },
        {
          "level": 3,
          "title": "什么时候使用Generator而不适合async",
          "slug": "什么时候使用generator而不适合async"
        },
        {
          "level": 3,
          "title": "箭头函数,优缺点,通过对象.方法调用的箭头函数里面this指向什么，如何改变指向",
          "slug": "箭头函数-优缺点-通过对象-方法调用的箭头函数里面this指向什么，如何改变指向"
        },
        {
          "level": 2,
          "title": "请求数据和跨域相关",
          "slug": "请求数据和跨域相关"
        },
        {
          "level": 3,
          "title": "从后台获取数据的方式",
          "slug": "从后台获取数据的方式"
        },
        {
          "level": 3,
          "title": "原生ajax对象,使用Promise对象封装",
          "slug": "原生ajax对象-使用promise对象封装"
        },
        {
          "level": 3,
          "title": "长轮询,短轮询",
          "slug": "长轮询-短轮询"
        },
        {
          "level": 3,
          "title": "什么是跨域,如何判断跨域,跨域的方法有哪些",
          "slug": "什么是跨域-如何判断跨域-跨域的方法有哪些"
        },
        {
          "level": 3,
          "title": "跨域携带cookies",
          "slug": "跨域携带cookies"
        },
        {
          "level": 3,
          "title": "什么是Jsonp,jsonp返回的内容",
          "slug": "什么是jsonp-jsonp返回的内容"
        },
        {
          "level": 3,
          "title": "CORS请求头的配置",
          "slug": "cors请求头的配置"
        },
        {
          "level": 3,
          "title": "不通过http如何传输数据",
          "slug": "不通过http如何传输数据"
        },
        {
          "level": 2,
          "title": "插件,算法和应用",
          "slug": "插件-算法和应用"
        },
        {
          "level": 3,
          "title": "遍历数组，对象",
          "slug": "遍历数组，对象"
        },
        {
          "level": 3,
          "title": "对象深拷贝",
          "slug": "对象深拷贝"
        },
        {
          "level": 3,
          "title": "一个列表，实现点击某个li标签，弹出元素内容",
          "slug": "一个列表，实现点击某个li标签，弹出元素内容"
        },
        {
          "level": 3,
          "title": "寻找数组缺失数",
          "slug": "寻找数组缺失数"
        },
        {
          "level": 3,
          "title": "原生javascript封装一个ajax方法",
          "slug": "原生javascript封装一个ajax方法"
        },
        {
          "level": 3,
          "title": "数组reduce方法实现去重",
          "slug": "数组reduce方法实现去重"
        },
        {
          "level": 3,
          "title": "随机乱序数组",
          "slug": "随机乱序数组"
        },
        {
          "level": 3,
          "title": "兼容IE和谷歌的事件绑定",
          "slug": "兼容ie和谷歌的事件绑定"
        },
        {
          "level": 3,
          "title": "原生js实现滚动效果",
          "slug": "原生js实现滚动效果"
        },
        {
          "level": 3,
          "title": "Js的类型判断有几种方式，区别，原理，使用场景",
          "slug": "js的类型判断有几种方式，区别，原理，使用场景"
        },
        {
          "level": 3,
          "title": "Bind,call,apply的原生实现",
          "slug": "bind-call-apply的原生实现"
        },
        {
          "level": 3,
          "title": "实现轮播图",
          "slug": "实现轮播图"
        },
        {
          "level": 3,
          "title": "尽可能多的方式实现数组去重",
          "slug": "尽可能多的方式实现数组去重"
        },
        {
          "level": 3,
          "title": "寻找数组最大数",
          "slug": "寻找数组最大数"
        },
        {
          "level": 3,
          "title": "实现一定时间内时间只触发一次",
          "slug": "实现一定时间内时间只触发一次"
        },
        {
          "level": 3,
          "title": "实现队列函数(先进先出),100毫秒后打印1,200毫秒后打印2以此类推",
          "slug": "实现队列函数-先进先出-100毫秒后打印1-200毫秒后打印2以此类推"
        },
        {
          "level": 3,
          "title": "实现类似模板字符串的功能",
          "slug": "实现类似模板字符串的功能"
        },
        {
          "level": 3,
          "title": "合并有序数组",
          "slug": "合并有序数组"
        },
        {
          "level": 3,
          "title": "九九乘法表",
          "slug": "九九乘法表"
        },
        {
          "level": 3,
          "title": "JavaScript字符串翻转",
          "slug": "javascript字符串翻转"
        },
        {
          "level": 3,
          "title": "URL解析的多种方式",
          "slug": "url解析的多种方式"
        },
        {
          "level": 3,
          "title": "时间显示，每分钟更新时间",
          "slug": "时间显示，每分钟更新时间"
        },
        {
          "level": 3,
          "title": "数组中出现频率前K和K个值（要求从KN的复杂度降到N复杂度）",
          "slug": "数组中出现频率前k和k个值（要求从kn的复杂度降到n复杂度）"
        },
        {
          "level": 3,
          "title": "原生JavaScript实现可复用蒙层弹窗",
          "slug": "原生javascript实现可复用蒙层弹窗"
        },
        {
          "level": 3,
          "title": "实现es6中的promise",
          "slug": "实现es6中的promise"
        },
        {
          "level": 3,
          "title": "实现es6中的generator",
          "slug": "实现es6中的generator"
        }
      ]
    },
    {
      "key": "v-a788058176b9a",
      "path": "/notebook/school/module.html",
      "lastUpdated": 1571138319000,
      "title": "前端工程化模块化",
      "headers": [
        {
          "level": 2,
          "title": "webpack",
          "slug": "webpack"
        },
        {
          "level": 3,
          "title": "webpack的loader和plugin区别和作用",
          "slug": "webpack的loader和plugin区别和作用"
        }
      ]
    },
    {
      "key": "v-95bbcc6f5cb81",
      "path": "/notebook/school/network.html",
      "lastUpdated": 1571138319000,
      "title": "计算机网络",
      "headers": [
        {
          "level": 2,
          "title": "基础知识",
          "slug": "基础知识"
        },
        {
          "level": 3,
          "title": "OSI七层模型,TCP协议在哪一层,HTTP协议在哪一层,tcp和HTTP的关系",
          "slug": "osi七层模型-tcp协议在哪一层-http协议在哪一层-tcp和http的关系"
        },
        {
          "level": 3,
          "title": "DNS查询过程",
          "slug": "dns查询过程"
        },
        {
          "level": 3,
          "title": "Tcp/ip三次握手、四次挥手的过程",
          "slug": "tcp-ip三次握手、四次挥手的过程"
        },
        {
          "level": 3,
          "title": "Tcp/IP的四层模型",
          "slug": "tcp-ip的四层模型"
        },
        {
          "level": 2,
          "title": "HTTP",
          "slug": "http"
        },
        {
          "level": 3,
          "title": "http协议,怎么理解无状态含义",
          "slug": "http协议-怎么理解无状态含义"
        },
        {
          "level": 3,
          "title": "http的header有哪些参数",
          "slug": "http的header有哪些参数"
        },
        {
          "level": 3,
          "title": "http1.0和1.1和2.0的区别以及改进",
          "slug": "http1-0和1-1和2-0的区别以及改进"
        },
        {
          "level": 3,
          "title": "http协议的缓存,强缓存和协商缓存,缓存控制机制",
          "slug": "http协议的缓存-强缓存和协商缓存-缓存控制机制"
        },
        {
          "level": 3,
          "title": "http状态码,常用状态码",
          "slug": "http状态码-常用状态码"
        },
        {
          "level": 3,
          "title": "https是什么,传输过程,有什么好处,原理和缺点",
          "slug": "https是什么-传输过程-有什么好处-原理和缺点"
        },
        {
          "level": 3,
          "title": "get和post的区别",
          "slug": "get和post的区别"
        },
        {
          "level": 3,
          "title": "Http协议是超文本传输协议,为什么特指文本而不是其他的",
          "slug": "http协议是超文本传输协议-为什么特指文本而不是其他的"
        }
      ]
    },
    {
      "key": "v-b19d227bcd9a3",
      "path": "/notebook/school/nodejs.html",
      "lastUpdated": 1571138319000,
      "title": "NodeJS",
      "headers": [
        {
          "level": 2,
          "title": "Express",
          "slug": "express"
        },
        {
          "level": 3,
          "title": "Express框架,express中间件",
          "slug": "express框架-express中间件"
        },
        {
          "level": 3,
          "title": "Node.js和浏览器js的区别",
          "slug": "node-js和浏览器js的区别"
        }
      ]
    },
    {
      "key": "v-8a81f8d39334",
      "path": "/notebook/school/otherknowlage.html",
      "lastUpdated": 1571138319000,
      "title": "其他知识",
      "headers": [
        {
          "level": 2,
          "title": "图片",
          "slug": "图片"
        },
        {
          "level": 3,
          "title": "图片格式区别,大小",
          "slug": "图片格式区别-大小"
        },
        {
          "level": 2,
          "title": "常用算法",
          "slug": "常用算法"
        },
        {
          "level": 3,
          "title": "压缩算法",
          "slug": "压缩算法"
        },
        {
          "level": 3,
          "title": "加密算法",
          "slug": "加密算法"
        }
      ]
    },
    {
      "key": "v-89902adee7bd1",
      "path": "/notebook/school/othertools.html",
      "lastUpdated": 1571138319000,
      "title": "其他工具",
      "headers": [
        {
          "level": 2,
          "title": "版本控制",
          "slug": "版本控制"
        },
        {
          "level": 3,
          "title": "Git常用命令",
          "slug": "git常用命令"
        }
      ]
    },
    {
      "key": "v-5ad41b34922c",
      "path": "/notebook/school/read.html",
      "lastUpdated": 1571307838000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-b96d6e928ab44",
      "path": "/notebook/school/structure.html",
      "lastUpdated": 1571138319000,
      "title": "数据结构",
      "headers": [
        {
          "level": 2,
          "title": "排序",
          "slug": "排序"
        },
        {
          "level": 3,
          "title": "什么是稳定排序,不稳定排序。哪些排序算法是稳定排序,不稳定排序",
          "slug": "什么是稳定排序-不稳定排序。哪些排序算法是稳定排序-不稳定排序"
        },
        {
          "level": 3,
          "title": "快排的思路,时间复杂度",
          "slug": "快排的思路-时间复杂度"
        },
        {
          "level": 3,
          "title": "排序的优化,非递归写法",
          "slug": "排序的优化-非递归写法"
        },
        {
          "level": 3,
          "title": "八个排序算法,时间复杂度,空间复杂度",
          "slug": "八个排序算法-时间复杂度-空间复杂度"
        },
        {
          "level": 3,
          "title": "二分查找",
          "slug": "二分查找"
        },
        {
          "level": 2,
          "title": "常用算法",
          "slug": "常用算法"
        },
        {
          "level": 3,
          "title": "用栈实现一个队列",
          "slug": "用栈实现一个队列"
        },
        {
          "level": 3,
          "title": "链表如何实现",
          "slug": "链表如何实现"
        },
        {
          "level": 3,
          "title": "树的遍历查询",
          "slug": "树的遍历查询"
        }
      ]
    },
    {
      "key": "v-f555e5b90a2a3",
      "path": "/readbook/literature/read.html",
      "lastUpdated": 1571138319000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-31596a2d57e7",
      "path": "/readbook/literature/%E4%BA%AE%E5%89%91.html",
      "lastUpdated": 1572934205000,
      "title": "亮剑",
      "frontmatter": {
        "title": "亮剑"
      }
    },
    {
      "key": "v-9f5620731d9e",
      "path": "/readbook/literature/%E5%91%BC%E5%85%B0%E6%B2%B3%E4%BC%A0.html",
      "lastUpdated": 1572934205000,
      "title": "呼兰河传",
      "frontmatter": {
        "title": "呼兰河传"
      }
    },
    {
      "key": "v-f32a6269dc472",
      "path": "/readbook/literature/%E5%B0%8F%E7%AA%97%E5%B9%BD%E8%AE%B0.html",
      "lastUpdated": 1572934205000,
      "title": "小窗幽记",
      "frontmatter": {
        "title": "小窗幽记"
      }
    },
    {
      "key": "v-addff56df7e3d",
      "path": "/readbook/literature/%E5%B9%B3%E5%87%A1%E7%9A%84%E4%B8%96%E7%95%8C.html",
      "lastUpdated": 1572934205000,
      "title": "平凡的世界",
      "frontmatter": {
        "title": "平凡的世界"
      }
    },
    {
      "key": "v-6e3f6197d9ef3",
      "path": "/readbook/literature/%E8%82%96%E7%94%B3%E5%85%8B%E7%9A%84%E6%95%91%E8%B5%8E.html",
      "lastUpdated": 1571138319000,
      "title": "肖申克的救赎",
      "frontmatter": {
        "title": "肖申克的救赎"
      }
    },
    {
      "key": "v-dd3dffcd9ec81",
      "path": "/readbook/other/read.html",
      "lastUpdated": 1571138319000,
      "title": "介绍",
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-573628b494aca",
      "path": "/readbook/other/%E4%BD%A0%E7%9A%84%E7%81%AF%E4%BA%AE%E7%9D%80%E5%90%97.html",
      "lastUpdated": 1571138319000,
      "title": "你的灯亮着吗?",
      "frontmatter": {
        "title": "你的灯亮着吗?"
      }
    },
    {
      "key": "v-aec3446d29de3",
      "path": "/readbook/other/%E5%BE%AE%E8%BD%AF%E7%9A%84%E6%A2%A6%E5%B7%A5%E5%8E%82.html",
      "lastUpdated": 1571138319000,
      "title": "微软的梦工厂",
      "frontmatter": {
        "title": "微软的梦工厂"
      }
    },
    {
      "key": "v-1b6d446940d47",
      "path": "/readbook/other/%E8%A8%80%E5%8F%B6%E4%B9%8B%E5%BA%AD.html",
      "lastUpdated": 1571138319000,
      "title": "言叶之庭",
      "frontmatter": {
        "title": "言叶之庭"
      }
    },
    {
      "key": "v-8cd1e03b0368",
      "path": "/readbook/poetry/read.html",
      "lastUpdated": 1574161526000,
      "title": "介绍",
      "headers": [
        {
          "level": 2,
          "title": "唐",
          "slug": "唐"
        },
        {
          "level": 2,
          "title": "宋",
          "slug": "宋"
        },
        {
          "level": 2,
          "title": "明",
          "slug": "明"
        },
        {
          "level": 2,
          "title": "清",
          "slug": "清"
        }
      ],
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-1a638172bee48",
      "path": "/readbook/poetry/%E5%94%90%E5%AF%85.html",
      "lastUpdated": 1574161526000,
      "title": "唐寅",
      "frontmatter": {
        "title": "唐寅",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-3a076a9b78739",
      "path": "/readbook/poetry/%E5%94%90%E7%8F%99.html",
      "lastUpdated": 1574161526000,
      "title": "唐珙",
      "frontmatter": {
        "title": "唐珙",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-764a2a245c2da",
      "path": "/readbook/poetry/%E5%AE%8B%E7%A5%81.html",
      "lastUpdated": 1574161526000,
      "title": "宋祁",
      "frontmatter": {
        "title": "宋祁",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-5106a7dddc443",
      "path": "/readbook/poetry/%E5%B4%94%E6%8A%A4.html",
      "lastUpdated": 1574161526000,
      "title": "崔护",
      "frontmatter": {
        "title": "崔护",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-e96f9aa41d532",
      "path": "/readbook/poetry/%E5%BC%A0%E6%97%AD.html",
      "lastUpdated": 1574161526000,
      "title": "张旭",
      "frontmatter": {
        "title": "张旭",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-32e81d2ff456e",
      "path": "/readbook/poetry/%E6%99%8F%E5%87%A0%E9%81%93.html",
      "lastUpdated": 1574161526000,
      "title": "晏几道",
      "frontmatter": {
        "title": "晏几道",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-d3adaae8c032b",
      "path": "/readbook/poetry/%E6%9C%B1%E6%B7%91%E7%9C%9F.html",
      "lastUpdated": 1574161526000,
      "title": "朱淑真",
      "frontmatter": {
        "title": "朱淑真",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-a187fa017b03f",
      "path": "/readbook/poetry/%E6%9D%8E%E5%95%86%E9%9A%90.html",
      "lastUpdated": 1574161526000,
      "title": "李商隐",
      "frontmatter": {
        "title": "李商隐",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-241cc5e114bc7",
      "path": "/readbook/poetry/%E6%9D%8E%E6%B8%85%E7%85%A7.html",
      "lastUpdated": 1574161526000,
      "title": "李清照",
      "frontmatter": {
        "title": "李清照",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-c6dabd7fb1ccd",
      "path": "/readbook/poetry/%E6%9D%8E%E7%99%BD.html",
      "lastUpdated": 1574161526000,
      "title": "李白",
      "frontmatter": {
        "title": "李白",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-c29597c4c5c68",
      "path": "/readbook/poetry/%E6%9D%9C%E7%89%A7.html",
      "lastUpdated": 1574161526000,
      "title": "杜牧",
      "frontmatter": {
        "title": "杜牧",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-b4ccf18d6afbb",
      "path": "/readbook/poetry/%E6%9D%A8%E4%B8%87%E9%87%8C.html",
      "lastUpdated": 1574161526000,
      "title": "杨万里",
      "frontmatter": {
        "title": "杨万里",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-b8ce0778b0479",
      "path": "/readbook/poetry/%E6%B1%AA%E9%81%B5.html",
      "lastUpdated": 1574161526000,
      "title": "汪遵",
      "frontmatter": {
        "title": "汪遵",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-fc0591e68f0ad",
      "path": "/readbook/poetry/%E7%99%BD%E5%B1%85%E6%98%93.html",
      "lastUpdated": 1574161526000,
      "title": "白居易",
      "frontmatter": {
        "title": "白居易",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-244789b9c2d8c",
      "path": "/readbook/poetry/%E7%99%BD%E7%8E%89%E8%9F%BE.html",
      "lastUpdated": 1574161526000,
      "title": "白玉蟾",
      "frontmatter": {
        "title": "白玉蟾",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-2443cec619b41",
      "path": "/readbook/poetry/%E7%9F%B3%E6%B6%9B.html",
      "lastUpdated": 1574161526000,
      "title": "石涛",
      "frontmatter": {
        "title": "石涛",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-17433dcb7f94b",
      "path": "/readbook/poetry/%E7%A7%A6%E8%A7%82.html",
      "lastUpdated": 1574161526000,
      "title": "秦观",
      "frontmatter": {
        "title": "秦观",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-f087ddec2b6d1",
      "path": "/readbook/poetry/%E7%BA%B3%E5%85%B0%E6%80%A7%E5%BE%B7.html",
      "lastUpdated": 1574161526000,
      "title": "纳兰性德",
      "frontmatter": {
        "title": "纳兰性德",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-1c7b0c7a607bb",
      "path": "/readbook/poetry/%E8%8B%8F%E6%9B%BC%E6%AE%8A.html",
      "lastUpdated": 1574161526000,
      "title": "苏曼殊",
      "frontmatter": {
        "title": "苏曼殊",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-b349ef08b123c",
      "path": "/readbook/poetry/%E8%8B%8F%E8%BD%BC.html",
      "lastUpdated": 1574161526000,
      "title": "苏轼",
      "frontmatter": {
        "title": "苏轼",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-ee2936461e7a2",
      "path": "/readbook/poetry/%E8%92%8B%E6%8D%B7.html",
      "lastUpdated": 1574161526000,
      "title": "蒋捷",
      "frontmatter": {
        "title": "蒋捷",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-6258296f5af1f",
      "path": "/readbook/poetry/%E9%99%86%E9%BE%9F%E8%92%99.html",
      "lastUpdated": 1574319777000,
      "title": "陆龟蒙",
      "frontmatter": {
        "title": "陆龟蒙",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-3bad0a92b638a",
      "path": "/readbook/poetry/%E9%99%88%E9%99%B6.html",
      "lastUpdated": 1574161526000,
      "title": "陈陶",
      "frontmatter": {
        "title": "陈陶",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-480287186c706",
      "path": "/readbook/poetry/%E9%9F%A6%E5%BA%84.html",
      "lastUpdated": 1574161526000,
      "title": "韦庄",
      "frontmatter": {
        "title": "韦庄",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-7f84d6eeae8df",
      "path": "/readbook/technology/201principlesofsoftwaredevelopment.html",
      "lastUpdated": 1572934205000,
      "title": "《201 principles of software development》",
      "frontmatter": {
        "title": "《201 principles of software development》"
      }
    },
    {
      "key": "v-4fe401d44feff",
      "path": "/readbook/technology/CPrimerPlus.html",
      "lastUpdated": 1572251081000,
      "title": "《C Primer Plus》",
      "frontmatter": {
        "title": "《C Primer Plus》"
      }
    },
    {
      "key": "v-a45318284240a",
      "path": "/readbook/technology/CSS%E6%8F%AD%E7%A7%98.html",
      "lastUpdated": 1572934205000,
      "title": "《CSS揭秘》",
      "frontmatter": {
        "title": "《CSS揭秘》"
      }
    },
    {
      "key": "v-adeb88d32ff4e",
      "path": "/readbook/technology/MySQL%E5%BF%85%E7%9F%A5%E5%BF%85%E4%BC%9A.html",
      "lastUpdated": 1572934205000,
      "title": "《MySQL必知必会》",
      "frontmatter": {
        "title": "《MySQL必知必会》"
      }
    },
    {
      "key": "v-a28ea72aab6ed",
      "path": "/readbook/technology/NodeJs%E6%9D%A5%E4%B8%80%E6%89%93C%E5%8A%A0%E5%8A%A0%E6%89%A9%E5%B1%95.html",
      "lastUpdated": 1572934205000,
      "title": "《Node.js 来一打C++扩展》",
      "headers": [
        {
          "level": 2,
          "title": "CommonJS模块",
          "slug": "commonjs模块"
        },
        {
          "level": 3,
          "title": "require",
          "slug": "require"
        },
        {
          "level": 3,
          "title": "模块上下文",
          "slug": "模块上下文"
        },
        {
          "level": 3,
          "title": "模块标识",
          "slug": "模块标识"
        },
        {
          "level": 3,
          "title": "Node.js寻址算法",
          "slug": "node-js寻址算法"
        },
        {
          "level": 3,
          "title": "Node.js模块缓存机制",
          "slug": "node-js模块缓存机制"
        },
        {
          "level": 2,
          "title": "Node.js的包机制",
          "slug": "node-js的包机制"
        }
      ],
      "frontmatter": {
        "title": "《Node.js 来一打C++扩展》",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-5bfa2f2dc3539",
      "path": "/readbook/technology/Node%E5%8D%B3%E5%AD%A6%E5%8D%B3%E7%94%A8.html",
      "lastUpdated": 1572934205000,
      "title": "《Node即学即用》",
      "frontmatter": {
        "title": "《Node即学即用》"
      }
    },
    {
      "key": "v-f66e4893199d8",
      "path": "/readbook/technology/PHP%E6%A0%B8%E5%BF%83%E6%8A%80%E6%9C%AF%E4%B8%8E%E6%9C%80%E4%BD%B3%E5%AE%9E%E8%B7%B5.html",
      "lastUpdated": 1572934205000,
      "title": "《PHP核心技术与最佳实践》",
      "frontmatter": {
        "title": "《PHP核心技术与最佳实践》"
      }
    },
    {
      "key": "v-0b087f29695ea",
      "path": "/readbook/technology/css%E4%B8%96%E7%95%8C.html",
      "lastUpdated": 1572251081000,
      "title": "《CSS世界》",
      "frontmatter": {
        "title": "《CSS世界》",
        "sidebarDepth": 1
      }
    },
    {
      "key": "v-5bf8b51178f09",
      "path": "/readbook/technology/css%E7%A6%85%E6%84%8F%E8%8A%B1%E5%9B%AD.html",
      "lastUpdated": 1572251081000,
      "title": "《CSS禅意花园》",
      "frontmatter": {
        "title": "《CSS禅意花园》"
      }
    },
    {
      "key": "v-2201e7625f5e3",
      "path": "/readbook/technology/javascript%E5%87%BD%E6%95%B0%E5%BC%8F%E7%BC%96%E7%A8%8B.html",
      "lastUpdated": 1572934205000,
      "title": "《JavaScript函数式编程》",
      "frontmatter": {
        "title": "《JavaScript函数式编程》"
      }
    },
    {
      "key": "v-d4d0cfe4b4576",
      "path": "/readbook/technology/javascript%E5%BF%8D%E8%80%85%E7%A7%98%E7%B1%8D.html",
      "lastUpdated": 1572934205000,
      "title": "《JavaScript忍者秘籍》",
      "frontmatter": {
        "title": "《JavaScript忍者秘籍》"
      }
    },
    {
      "key": "v-84e2cf1b548e4",
      "path": "/readbook/technology/javascript%E8%AF%AD%E8%A8%80%E7%B2%BE%E7%B2%B9.html",
      "lastUpdated": 1572251081000,
      "title": "《JavaScript语言精粹》",
      "frontmatter": {
        "title": "《JavaScript语言精粹》"
      }
    },
    {
      "key": "v-af44e9fe074d5",
      "path": "/readbook/technology/javascript%E9%AB%98%E7%BA%A7%E7%A8%8B%E5%BA%8F%E8%AE%BE%E8%AE%A1.html",
      "lastUpdated": 1572251081000,
      "title": "《JavaScript高级程序设计》",
      "frontmatter": {
        "title": "《JavaScript高级程序设计》"
      }
    },
    {
      "key": "v-7c455690cc213",
      "path": "/readbook/technology/js%E6%9D%83%E5%A8%81%E6%8C%87%E5%8D%97.html",
      "lastUpdated": 1572251081000,
      "title": "《JS权威指南》",
      "frontmatter": {
        "title": "《JS权威指南》"
      }
    },
    {
      "key": "v-f5d67dabb6d2e",
      "path": "/readbook/technology/read.html",
      "lastUpdated": 1572251081000,
      "title": "介绍",
      "headers": [
        {
          "level": 2,
          "title": "HTML",
          "slug": "html"
        },
        {
          "level": 2,
          "title": "CSS",
          "slug": "css"
        },
        {
          "level": 2,
          "title": "JS",
          "slug": "js"
        },
        {
          "level": 2,
          "title": "Node",
          "slug": "node"
        },
        {
          "level": 2,
          "title": "前端生态",
          "slug": "前端生态"
        },
        {
          "level": 2,
          "title": "数据库",
          "slug": "数据库"
        },
        {
          "level": 2,
          "title": "代码质量",
          "slug": "代码质量"
        },
        {
          "level": 2,
          "title": "C语言",
          "slug": "c语言"
        },
        {
          "level": 2,
          "title": "PHP",
          "slug": "php"
        },
        {
          "level": 2,
          "title": "JAVA",
          "slug": "java"
        },
        {
          "level": 2,
          "title": "内功",
          "slug": "内功"
        }
      ],
      "frontmatter": {
        "title": "介绍"
      }
    },
    {
      "key": "v-9632b63c1fc69",
      "path": "/readbook/technology/vuejs%E5%AE%9E%E6%88%98.html",
      "lastUpdated": 1572934205000,
      "title": "《Vue.js实战》",
      "frontmatter": {
        "title": "《Vue.js实战》"
      }
    },
    {
      "key": "v-5209b8eb78f3b",
      "path": "/readbook/technology/%E4%BB%A3%E7%A0%81%E5%A4%A7%E5%85%A8.html",
      "lastUpdated": 1572934205000,
      "title": "《代码大全》",
      "frontmatter": {
        "title": "《代码大全》"
      }
    },
    {
      "key": "v-160b5cac184fb",
      "path": "/readbook/technology/%E4%BB%A3%E7%A0%81%E6%95%B4%E6%B4%81%E4%B9%8B%E9%81%93.html",
      "lastUpdated": 1572934205000,
      "title": "《代码整洁之道》",
      "frontmatter": {
        "title": "《代码整洁之道》"
      }
    },
    {
      "key": "v-9f6c503bb98b9",
      "path": "/readbook/technology/%E4%BD%A0%E4%B8%8D%E7%9F%A5%E9%81%93%E7%9A%84javascript.html",
      "lastUpdated": 1572251081000,
      "title": "《你不知道的JavaScript》",
      "frontmatter": {
        "title": "《你不知道的JavaScript》"
      }
    },
    {
      "key": "v-3d420106039bf",
      "path": "/readbook/technology/%E5%89%91%E6%8C%87offer.html",
      "lastUpdated": 1572251081000,
      "title": "《剑指Offer》",
      "frontmatter": {
        "title": "《剑指Offer》"
      }
    },
    {
      "key": "v-928be60a9c164",
      "path": "/readbook/technology/%E5%9B%BE%E8%A7%A3http.html",
      "lastUpdated": 1572251081000,
      "title": "《图解HTTP》",
      "frontmatter": {
        "title": "《图解HTTP》"
      }
    },
    {
      "key": "v-1030066abdfd",
      "path": "/readbook/technology/%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84%E4%B8%8E%E7%AE%97%E6%B3%95%E5%88%86%E6%9E%90C%E8%AF%AD%E8%A8%80%E6%8F%8F%E8%BF%B0.html",
      "lastUpdated": 1572251081000,
      "title": "《数据结构与算法分析-C语言描述》",
      "frontmatter": {
        "title": "《数据结构与算法分析-C语言描述》"
      }
    },
    {
      "key": "v-543e24915913b",
      "path": "/readbook/technology/%E6%B7%B1%E5%85%A5%E6%B5%85%E5%87%BANodeJs.html",
      "lastUpdated": 1572934205000,
      "title": "《深入浅出Node.js》",
      "frontmatter": {
        "title": "《深入浅出Node.js》"
      }
    },
    {
      "key": "v-413bfdff594f2",
      "path": "/readbook/technology/%E6%B7%B1%E5%85%A5%E6%B5%85%E5%87%BAWebpack.html",
      "lastUpdated": 1572934205000,
      "title": "《深入浅出Webpack》",
      "frontmatter": {
        "title": "《深入浅出Webpack》"
      }
    },
    {
      "key": "v-d1f2f33162b34",
      "path": "/readbook/technology/%E7%96%AF%E7%8B%82Java%E8%AE%B2%E4%B9%89.html",
      "lastUpdated": 1572934205000,
      "title": "《疯狂Java讲义》",
      "frontmatter": {
        "title": "《疯狂Java讲义》"
      }
    },
    {
      "key": "v-f8230ca8c7013",
      "path": "/readbook/technology/%E7%AE%97%E6%B3%95%E5%AF%BC%E8%AE%BA.html",
      "lastUpdated": 1572934205000,
      "title": "《算法导论》",
      "frontmatter": {
        "title": "《算法导论》"
      }
    },
    {
      "key": "v-5b9e4573acf07",
      "path": "/readbook/technology/%E7%BC%96%E5%86%99%E5%8F%AF%E8%AF%BB%E5%86%99%E4%BB%A3%E7%A0%81%E7%9A%84%E8%89%BA%E6%9C%AF.html",
      "lastUpdated": 1572934205000,
      "title": "《编写可读写代码的艺术》",
      "frontmatter": {
        "title": "《编写可读写代码的艺术》"
      }
    },
    {
      "key": "v-0bcbc730b6d8f",
      "path": "/readbook/technology/%E7%BC%96%E7%A0%81%E9%9A%90%E5%8C%BF%E5%9C%A8%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%A1%AC%E4%BB%B6%E8%83%8C%E5%90%8E%E7%9A%84%E8%AF%AD%E8%A8%80.html",
      "lastUpdated": 1572251081000,
      "title": "《编码-隐匿在计算机硬件背后的语言》",
      "frontmatter": {
        "title": "《编码-隐匿在计算机硬件背后的语言》"
      }
    },
    {
      "key": "v-4270ba98a49ee",
      "path": "/readbook/technology/%E9%94%8B%E5%88%A9%E7%9A%84jquery.html",
      "lastUpdated": 1572251081000,
      "title": "《锋利的jQuery》",
      "frontmatter": {
        "title": "《锋利的jQuery》"
      }
    },
    {
      "key": "v-5b879cac43fb8",
      "path": "/readbook/technology/%E9%AB%98%E6%80%A7%E8%83%BDMySQL.html",
      "lastUpdated": 1572934205000,
      "title": "《高性能MySQL》",
      "frontmatter": {
        "title": "《高性能MySQL》"
      }
    },
    {
      "key": "v-1205a2b58a78d",
      "path": "/readbook/technology/%E9%AB%98%E6%80%A7%E8%83%BD%E7%9A%84javascript.html",
      "lastUpdated": 1572251081000,
      "title": "《高性能的javaScript》",
      "frontmatter": {
        "title": "《高性能的javaScript》"
      }
    }
  ],
  "themeConfig": {
    "repo": "yuhongjing/library",
    "sidebarDepth": 2,
    "lastUpdated": "上次更新",
    "editLinks": true,
    "docsDir": "docs",
    "searchMaxSuggestions": 15,
    "locales": {
      "/": {
        "nav": [
          {
            "text": "手册",
            "items": [
              {
                "text": "校招面经",
                "link": "/notebook/school/read"
              },
              {
                "text": "JS Tricks",
                "link": "/notebook/jsTricks/read"
              },
              {
                "text": "CSS Tricks",
                "link": "/notebook/cssTricks/read"
              },
              {
                "text": "算法整理",
                "link": "/notebook/algorithm/read"
              },
              {
                "text": "北漂日志",
                "link": "/notebook/difters/read"
              }
            ]
          },
          {
            "text": "日常记录",
            "items": [
              {
                "text": "闲情随记",
                "link": "/blog/article/read"
              },
              {
                "text": "工具技巧",
                "link": "/blog/tools/read"
              },
              {
                "text": "命令整理",
                "link": "/blog/command/read"
              },
              {
                "text": "必备软件",
                "link": "/blog/software/read"
              },
              {
                "text": "文档综合",
                "link": "/blog/other/read"
              }
            ]
          },
          {
            "text": "知识整理",
            "items": [
              {
                "text": "知识脑图",
                "link": "/knowledge/mindmap/read"
              },
              {
                "text": "前端知识",
                "link": "/knowledge/web/read"
              },
              {
                "text": "后端知识",
                "link": "/knowledge/backend/read"
              },
              {
                "text": "数据库知识",
                "link": "/knowledge/database/read"
              },
              {
                "text": "算法知识",
                "link": "/knowledge/algorithm/read"
              },
              {
                "text": "数据结构知识",
                "link": "/knowledge/structure/read"
              },
              {
                "text": "设计模式知识",
                "link": "/knowledge/design/read"
              },
              {
                "text": "网络知识",
                "link": "/knowledge/network/read"
              },
              {
                "text": "硬件知识",
                "link": "/knowledge/hard/read"
              },
              {
                "text": "操作系统知识",
                "link": "/knowledge/system/read"
              },
              {
                "text": "环境配置",
                "link": "/knowledge/config/read"
              },
              {
                "text": "项目架构",
                "link": "/knowledge/architecture/read"
              },
              {
                "text": "英语知识",
                "link": "/knowledge/english/read"
              }
            ]
          },
          {
            "text": "源码解析",
            "items": [
              {
                "text": "Loadsh",
                "link": "/code/loadsh/read"
              },
              {
                "text": "Webpack",
                "link": "/code/webpack/read"
              },
              {
                "text": "Zepto",
                "link": "/code/zepto/read"
              }
            ]
          },
          {
            "text": "读书笔记",
            "items": [
              {
                "text": "技术书籍",
                "link": "/readbook/technology/read"
              },
              {
                "text": "文学书籍",
                "link": "/readbook/literature/read"
              },
              {
                "text": "诗词歌曲",
                "link": "/readbook/poetry/read"
              },
              {
                "text": "杂项书籍",
                "link": "/readbook/other/read"
              }
            ]
          },
          {
            "text": "其他项目",
            "items": [
              {
                "text": "旅行日志",
                "link": "https://yuhongjing.github.io/trip/#/"
              },
              {
                "text": "小裕博客",
                "link": "https://pxy1996.github.io/"
              },
              {
                "text": "旧博客",
                "link": "https://yuhongjing.github.io"
              }
            ]
          }
        ],
        "sidebar": {
          "/notebook/school/": [
            {
              "title": "校招面经",
              "collapsable": false,
              "children": [
                "read",
                "html",
                "css",
                "javascript",
                "freamwork",
                "nodejs",
                "module",
                "database",
                "structure",
                "network",
                "othertools",
                "otherknowlage"
              ]
            }
          ],
          "/notebook/jsTricks/": [
            {
              "title": "JS Tricks",
              "collapsable": false,
              "children": [
                "read",
                "string",
                "number",
                "boolean",
                "array",
                "object",
                "function",
                "dom",
                "code"
              ]
            }
          ],
          "/notebook/cssTricks/": [
            {
              "title": "CSS Tricks",
              "collapsable": false,
              "children": [
                "read",
                "layout",
                "behavior",
                "color",
                "figure",
                "component"
              ]
            }
          ],
          "/notebook/algorithm/": [
            {
              "title": "算法整理",
              "collapsable": false,
              "children": [
                "read",
                "shuffle"
              ]
            }
          ],
          "/notebook/difters/": [
            {
              "title": "北漂日志",
              "collapsable": false,
              "children": [
                "read",
                "第一章"
              ]
            }
          ],
          "/blog/article/": [
            {
              "title": "闲情随记",
              "collapsable": false,
              "children": [
                "read",
                "five-years-plan",
                "人是怎么废掉的"
              ]
            },
            {
              "title": "纪念意义",
              "children": [
                "BATFighting",
                "秋招总结",
                "第二个目标"
              ]
            }
          ],
          "/blog/tools/": [
            {
              "title": "工具技巧",
              "collapsable": false,
              "children": [
                "read",
                "git连接多仓库",
                "win10激活",
                "移动端调试",
                "git修改作者",
                "PicGo配合Github图床"
              ]
            }
          ],
          "/blog/command/": [
            {
              "title": "常用命令",
              "collapsable": false,
              "children": [
                "read"
              ]
            },
            {
              "title": "命令语言类",
              "children": [
                "git",
                "vim",
                "linux",
                "markdown"
              ]
            },
            {
              "title": "软件快捷键类",
              "children": [
                "iterm2",
                "Alfred4",
                "vscode",
                "Tmux",
                "Mac"
              ]
            }
          ],
          "/blog/software/": [
            {
              "title": "必备软件",
              "collapsable": false,
              "children": [
                "read",
                "Mac必备应用",
                "Vscode必备插件",
                "Node插件",
                "Webpack插件",
                "Chrome插件"
              ]
            }
          ],
          "/blog/other/": [
            {
              "title": "文档综合",
              "collapsable": false,
              "children": [
                "read",
                "代码产出"
              ]
            }
          ],
          "/knowledge/mindmap/": [
            {
              "title": "知识脑图",
              "collapsable": false,
              "children": [
                "read",
                "高效学习",
                "自我管理",
                "端正学习态度",
                "源头原理和知识地图",
                "深度归纳和坚持实践",
                "学习和阅读源码",
                "面对枯燥和量大的知识",
                "如何阅读一本英文书",
                "理论学科",
                "编程语言",
                "编程素养",
                "系统知识",
                "软件设计",
                "系统底层",
                "前端基础与底层原理",
                "异步IO与Lock_Free",
                "正式入门",
                "零基础启蒙"
              ]
            },
            {
              "title": "程序员练级攻略",
              "children": [
                "程序员练级攻略开篇",
                "程序员练级攻略入门",
                "程序员练级攻略进阶"
              ]
            }
          ],
          "/knowledge/web/": [
            {
              "title": "前端知识",
              "collapsable": false,
              "children": [
                "read"
              ]
            },
            {
              "title": "HTML",
              "children": [
                "html",
                "html-滚动条",
                "html-交叉观察者"
              ]
            },
            {
              "title": "CSS",
              "children": [
                "css"
              ]
            },
            {
              "title": "JavaScript",
              "children": [
                "javascript"
              ]
            },
            {
              "title": "Vue",
              "children": [
                "vue",
                "vue-编码规范",
                "vue-devtool"
              ]
            },
            {
              "title": "React",
              "children": [
                "react",
                "react-编码规范"
              ]
            },
            {
              "title": "Webpack",
              "children": [
                "webpack"
              ]
            },
            {
              "title": "TypeScript",
              "children": [
                "typescript"
              ]
            }
          ],
          "/knowledge/backend/": [
            {
              "title": "后端知识",
              "collapsable": false,
              "children": [
                "read",
                "php"
              ]
            }
          ],
          "/knowledge/database/": [
            {
              "title": "数据库知识",
              "collapsable": false,
              "children": [
                "read"
              ]
            },
            {
              "title": "MySQL",
              "children": [
                "MySQL介绍",
                "MySQL开发规范"
              ]
            }
          ],
          "/knowledge/algorithm/": [
            {
              "title": "算法知识",
              "collapsable": false,
              "children": [
                "read"
              ]
            },
            {
              "title": "排序",
              "children": [
                "复杂度",
                "排序概览",
                "冒泡排序",
                "选择排序",
                "快速排序",
                "插入排序",
                "归并排序",
                "堆排序"
              ]
            },
            {
              "title": "分治",
              "children": [
                "数组逆序对"
              ]
            }
          ],
          "/knowledge/structure/": [
            {
              "title": "数据结构知识",
              "collapsable": false,
              "children": [
                "read",
                "算法",
                "线性表"
              ]
            }
          ],
          "/knowledge/design/": [
            {
              "title": "设计模式知识",
              "collapsable": false,
              "children": [
                "read"
              ]
            },
            {
              "title": "基础原则",
              "children": [
                "solid",
                "迪米特法则"
              ]
            },
            {
              "title": "创建型模式",
              "children": [
                "简单工厂模式",
                "工厂方法模式",
                "原型模式"
              ]
            },
            {
              "title": "行为型模式",
              "children": [
                "策略模式",
                "模板方法模式"
              ]
            },
            {
              "title": "结构型模式",
              "children": [
                "装饰者模式",
                "代理模式",
                "外观模式"
              ]
            }
          ],
          "/knowledge/network/": [
            {
              "title": "网络知识",
              "collapsable": false,
              "children": [
                "read"
              ]
            }
          ],
          "/knowledge/hard/": [
            {
              "title": "硬件知识",
              "collapsable": false,
              "children": [
                "read"
              ]
            }
          ],
          "/knowledge/system/": [
            {
              "title": "操作系统知识",
              "collapsable": false,
              "children": [
                "read"
              ]
            }
          ],
          "/knowledge/config/": [
            {
              "title": "环境配置",
              "collapsable": false,
              "children": [
                "read",
                "eslint",
                "php",
                "mysql",
                "c"
              ]
            }
          ],
          "/knowledge/architecture/": [
            {
              "title": "架构整理",
              "collapsable": false,
              "children": [
                "read"
              ]
            }
          ],
          "/knowledge/english/": [
            {
              "title": "英语知识",
              "collapsable": false,
              "children": [
                "read"
              ]
            },
            {
              "title": "语法",
              "children": [
                "英语语法新思维初级"
              ]
            }
          ],
          "/code/loadsh/": [
            {
              "title": "Loadsh源码解析",
              "collapsable": false,
              "children": [
                "read",
                "array",
                "collection",
                "date",
                "function",
                "lang",
                "math",
                "number",
                "object",
                "seq",
                "string",
                "util",
                "properties",
                "methods"
              ]
            }
          ],
          "/code/webpack/": [
            {
              "title": "Webpack源码解析",
              "collapsable": false,
              "children": [
                "read"
              ]
            }
          ],
          "/code/zepto/": [
            {
              "title": "Zepto源码解析",
              "collapsable": false,
              "children": [
                "read",
                "structure"
              ]
            }
          ],
          "/readbook/technology/": [
            {
              "title": "技术书籍",
              "collapsable": false,
              "children": [
                "read"
              ]
            },
            {
              "title": "HTML",
              "children": []
            },
            {
              "title": "CSS",
              "children": [
                "css世界",
                "css禅意花园",
                "CSS揭秘"
              ]
            },
            {
              "title": "JS",
              "children": [
                "js权威指南",
                "javascript语言精粹",
                "javascript高级程序设计",
                "javascript函数式编程",
                "你不知道的javascript",
                "高性能的javascript",
                "javascript忍者秘籍"
              ]
            },
            {
              "title": "Node",
              "children": [
                "Node即学即用",
                "深入浅出NodeJs",
                "NodeJs来一打C加加扩展"
              ]
            },
            {
              "title": "前端生态",
              "children": [
                "锋利的jquery",
                "深入浅出Webpack",
                "vuejs实战"
              ]
            },
            {
              "title": "数据库",
              "children": [
                "MySQL必知必会",
                "高性能MySQL"
              ]
            },
            {
              "title": "代码质量",
              "children": [
                "201principlesofsoftwaredevelopment",
                "编写可读写代码的艺术",
                "代码大全",
                "代码整洁之道"
              ]
            },
            {
              "title": "C语言",
              "children": [
                "CPrimerPlus"
              ]
            },
            {
              "title": "PHP",
              "children": [
                "PHP核心技术与最佳实践"
              ]
            },
            {
              "title": "Java",
              "children": [
                "疯狂Java讲义"
              ]
            },
            {
              "title": "内功",
              "children": [
                "图解http",
                "编码隐匿在计算机硬件背后的语言",
                "剑指offer",
                "数据结构与算法分析C语言描述",
                "算法导论"
              ]
            }
          ],
          "/readbook/literature/": [
            {
              "title": "文学书籍",
              "collapsable": false,
              "children": [
                "read",
                "肖申克的救赎",
                "小窗幽记",
                "呼兰河传",
                "平凡的世界",
                "亮剑"
              ]
            }
          ],
          "/readbook/poetry/": [
            {
              "title": "诗词歌曲",
              "collapsable": false,
              "children": [
                "read"
              ]
            },
            {
              "title": "唐代",
              "children": [
                "李白",
                "白居易",
                "陆龟蒙",
                "韦庄",
                "张旭",
                "杜牧",
                "李商隐",
                "汪遵",
                "陈陶",
                "崔护"
              ]
            },
            {
              "title": "宋代",
              "children": [
                "宋祁",
                "白玉蟾",
                "杨万里",
                "朱淑真",
                "李清照",
                "秦观",
                "苏轼",
                "晏几道",
                "蒋捷"
              ]
            },
            {
              "title": "明代",
              "children": [
                "唐寅",
                "唐珙"
              ]
            },
            {
              "title": "清代",
              "children": [
                "苏曼殊",
                "石涛",
                "纳兰性德"
              ]
            }
          ],
          "/readbook/other/": [
            {
              "title": "杂项书籍",
              "collapsable": false,
              "children": [
                "read",
                "你的灯亮着吗",
                "微软的梦工厂",
                "言叶之庭"
              ]
            }
          ]
        }
      }
    }
  },
  "locales": {
    "/": {
      "lang": "zh-CN",
      "title": "余鸿靖的手册小站",
      "description": "手册小站目前有187835字，继续努力吧",
      "serviceWorker": {
        "updatePopup": {
          "message": "发现新内容更新.",
          "buttonText": "刷新"
        }
      }
    }
  }
}